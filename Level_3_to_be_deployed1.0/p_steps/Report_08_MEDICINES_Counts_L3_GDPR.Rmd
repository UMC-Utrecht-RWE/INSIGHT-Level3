...
subtitle: "GDPR: Counts of medicine use in the study population"
output: 
  html_document:
    theme: spacelab
    toc: true
...
if (subpopulations_present=="No"){
  if(length(list.files(paste0(med_dir,"GDPR/"), pattern="medicines_my_atc_1_masked.csv"))>0){
tab10_med<-fread(paste0(med_dir,"GDPR/", list.files(paste0(med_dir,"GDPR/"), pattern="medicines_my_atc_1_masked.csv")))
if(tab10_med[,.N]>0){
...
datatable(head(tab10_med,12000), options = list(scrollX=T), filter = "top")
 } else {
        datatable(tab10_med, options = list(scrollX=T), filter = "top")
 }
} else {
  tab10_med<-NULL
  ...
  print("This table cannot be produced based on the available data.")
    ...
    if(length(list.files(paste0(med_dir,subpopulations_names[a], "/GDPR/"), pattern="medicines_my_atc_1_masked.csv"))>0){
  tab10_med<-fread(paste0(med_dir,subpopulations_names[a], "/GDPR/", list.files(paste0(med_dir,subpopulations_names[a], "/GDPR/"), pattern="medicines_my_atc_1_masked.csv")))
  if (tab10_med[,.N]>0){
  setcolorder(tab10_med,c("atc_code_1", "count","total", "meaning", "year", "data_access_provider", "data_source"))
      if(tab10_med[,.N]>12000){  
    print("If the table contanins more than 12000 records it can not be displayed in the report. For the complete table, go to g_output/MEDICINES/medicines_my_atc_1_masked.csv")
...
if (subpopulations_present=="No"){
  if(length(list.files(paste0(med_dir,"GDPR/"), pattern="medicines_my_atc_4_masked.csv"))>0){
tab12_med<-fread(paste0(med_dir,"GDPR/", list.files(paste0(med_dir,"GDPR/"), pattern="medicines_my_atc_4_masked.csv")))
rec<-records_categories[,c("range", "label_records")]
setnames(rec, "range", "no_records")
tab12_med<-merge.data.table(tab12_med, rec, by="no_records", all.x=T, allow.cartesian = T)
setcolorder(tab12_med,c("sex","year","atc_4","no_records","no_users","median_records","mean_records","label_records","meaning","atc_1","atc_3"))
if(tab12_med[,.N]>0){
  if(tab12_med[,.N]>12000){
    ...
    datatable(head(tab12_med,12000), options = list(scrollX=T), filter = "top")
  } else {
datatable(tab12_med, options = list(scrollX=T), filter = "top")
  }
} else {
  tab12_med<-NULL
    print("This table cannot be produced based on the available data.")
    ...
    if(length(list.files(paste0(med_dir,subpopulations_names[a], "/GDPR/"), pattern="medicines_my_atc_4_masked.csv"))>0){
  tab12_med<-fread(paste0(med_dir,subpopulations_names[a], "/GDPR/", list.files(paste0(med_dir,subpopulations_names[a], "/GDPR/"), pattern="medicines_my_atc_4_masked.csv")))
rec<-records_categories[,c("range", "label_records")]
setnames(rec, "range", "no_records")
tab12_med<-merge.data.table(tab12_med, rec, by="no_records", all.x=T, allow.cartesian = T)
setcolorder(tab12_med,c("sex","year","atc_4","no_records","no_users","median_records","mean_records","label_records","meaning","atc_1","atc_3"))
...
if (tab12_med[,.N] != 0){
fig.tab12_med<-vector(mode="list", length=length(unique(tab12_med[,meaning])))
for(i in 1:length(unique(tab12_med[,meaning]))){
  tab12_med<-tab12_med[,label_records:=as.numeric(label_records)]
fig.tab12_med[[i]]<-ggplotly(ggplot(tab12_med[meaning==unique(tab12_med[,meaning])[i]], aes(x = year, y = label_records, group=atc_4)) +
                           geom_line(aes(color=atc_4)) +
                           geom_point(aes(color=atc_4)) + 
                            facet_wrap(~ sex, ncol=2, scales = "fixed") +
                               ggtitle(paste0("Number of records by ATC code, year and meaning:", unique(tab12_med[,meaning])[i])) + 
                               xlab("Year") +
                               ylab("Count")+
                             theme_classic() +
                             geom_text_repel(aes(label = no_records), hjust = -0.2, vjust = 0.5) +
                             guides(shape = guide_legend(override.aes = list(size = 0.3))) +
                               theme(text=element_text(size=10),
                                 axis.text.x = element_text(angle = 90, hjust = 1),
                                     axis.title.x = element_text(colour = "#76b82a"),
...
for (i in 1: length(fig.tab12_med)){
fig.tab12_med[[i]][['x']][['layout']][['annotations']][[1]][['y']] <--0.15
}
```

```{r display_tab12_graph_med, echo=FALSE}
if(!is.null(tab12_med)){
htmltools::tagList(list(fig.tab12_med))
} else {
  print("This table cannot be produced based on the available data.") 
}
```
...
if (subpopulations_present=="No"){
  if(length(list.files(paste0(med_dir,"GDPR/"), pattern="medicines_my_atc_7_masked.csv"))>0){
tab13_med<-fread(paste0(med_dir,"GDPR/", list.files(paste0(med_dir,"GDPR/"), pattern="medicines_my_atc_7_masked.csv")))
rec<-records_categories[,c("range", "label_records")]
setnames(rec, "range", "no_records")
tab13_med<-merge.data.table(tab13_med, rec, by="no_records", all.x=T, allow.cartesian = T)
setcolorder(tab13_med,c("sex","year","atc_7", "no_records","no_users", "median_records","mean_records","label_records","meaning", "data_access_provider", "data_source"))
...
if(tab13_med[,.N]>0){
  if(tab13_med[,.N]>12000){
    ...
    datatable(head(tab13_med,12000), options = list(scrollX=T), filter = "top")
  } else {
  datatable(tab13_med, options = list(scrollX=T), filter = "top")
  }
} else {
  tab13_med<-NULL
    print("This table cannot be produced based on the available data.")
    ...
    if(length(list.files(paste0(med_dir,subpopulations_names[a], "/GDPR/"), pattern="medicines_my_atc_7_masked.csv"))>0){
  tab13_med<-fread(paste0(med_dir,subpopulations_names[a], "/GDPR/", list.files(paste0(med_dir,subpopulations_names[a], "/GDPR/"), pattern="medicines_my_atc_7_masked.csv")))
  rec<-records_categories[,c("range", "label_records")]
setnames(rec, "range", "no_records")
tab13_med<-merge.data.table(tab13_med, rec, by="no_records", all.x=T, allow.cartesian = T)
setcolorder(tab13_med,c("sex","year","atc_7", "no_records","no_users", "median_records","mean_records","label_records","meaning", "data_access_provider", "data_source"))
...
```{r tab13_records_graph, echo=FALSE, warning=FALSE}
if(!is.null(tab13_med)){
tab13_med_graph<-tab13_med[,.(sex,year,atc_7,label_records)]
tab13_med_graph<-tab13_med_graph[order(year,label_records)]
tab13_med_graph[,year:=as.factor(year)]
}

if(!is.null(tab13_med_graph)){
if (tab13_med_graph[,.N] != 0){
ggplotly(ggplot(tab13_med_graph, aes(x = year, y = label_records, group=atc_7)) +
                           ...
                           geom_point(aes(color=atc_7)) + 
                            facet_wrap(~ sex, ncol=2, scales = "fixed") +
                               ...
                               ylab("Count")+
                             theme_classic() +
           scale_y_continuous(labels = function(x) format(x, digits = 1, scientific=F)) +
                             guides(shape = guide_legend(override.aes = list(size = 0.3))) +
                               theme(text=element_text(size=10),
                                 axis.text.x = element_text(angle = 90, hjust = 1),
                                     axis.title.x = element_text(colour = "#76b82a"),
