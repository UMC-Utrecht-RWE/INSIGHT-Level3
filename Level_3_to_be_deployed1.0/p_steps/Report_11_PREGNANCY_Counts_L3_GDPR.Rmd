...
subtitle: "GDPR: Counts of the Pregnancy study population"
output: 
  html_document:
    theme: spacelab
    toc: true
...
```{r preg_yam, echo=F}
if (subpopulations_present=="No"){
  if(length(list.files(paste0(preg_dir,"GDPR/"), pattern="pregnancy_counts_yam_masked.csv"))>0){
preg_yam<-fread(paste0(preg_dir,"GDPR/", list.files(paste0(preg_dir,"GDPR/"), pattern="pregnancy_counts_yam_masked.csv")))
...
datatable(head(preg_yam,12000), options = list(scrollX=T), filter = "top")
 } else {
        datatable(preg_yam, options = list(scrollX=T), filter = "top")
 }
} else {
  preg_yam<-NULL
  ...
  print("This table cannot be produced based on the available data.")
    ...
    if(length(list.files(paste0(preg_dir,subpopulations_names[a], "/GDPR/"), pattern="pregnancy_counts_yam_masked.csv"))>0){
  preg_yam<-fread(paste0(preg_dir,subpopulations_names[a], "/GDPR/", list.files(paste0(preg_dir,subpopulations_names[a], "/GDPR/"), pattern="pregnancy_counts_yam_masked.csv")))
...
if(!is.null(preg_yam)){
preg_yam[,event_definition_meaning:=paste(event_definition,meaning,sep=":")]
  fig.preg_yam_graph<-vector(mode="list", length=length(unique(preg_yam[,age_band])))
  for(i in 1:length(unique(preg_yam[,age_band]))){
  fig.preg_yam_graph[[i]]<-ggplotly(ggplot(preg_yam[age_band==unique(preg_yam[,age_band])[i]], aes(fill=event_definition_meaning, y=percentage, x=year)) + 
   geom_line(aes(color=event_definition_meaning),stat = "identity") +
  geom_point(aes(color=event_definition_meaning)) + 
    facet_wrap( ~ age_band) +
    ggtitle(paste0("Pregnancy counts by meaning, year, event definition and age band:",unique(preg_yam[,age_band])[i])) +
    xlab("Year") +
     ylab("Count")+
...
```{r preg_ym, echo=F}
if (subpopulations_present=="No"){
  if(length(list.files(paste0(preg_dir,"GDPR/"), pattern="pregnancy_counts_ym_masked.csv"))>0){
preg_ym<-fread(paste0(preg_dir,"GDPR/", list.files(paste0(preg_dir,"GDPR/"), pattern="pregnancy_counts_ym_masked.csv")))
...
datatable(head(preg_ym,12000), options = list(scrollX=T), filter = "top")
 } else {
        datatable(preg_ym, options = list(scrollX=T), filter = "top")
 }
} else {
  preg_ym<-NULL
  print("This table cannot be produced based on the available data.")
  ...
  print("This table cannot be produced based on the available data.")
    ...
    if(length(list.files(paste0(preg_dir,subpopulations_names[a], "/GDPR/"), pattern="pregnancy_counts_ym_masked.csv"))>0){
  preg_ym<-fread(paste0(preg_dir,subpopulations_names[a], "/GDPR/", list.files(paste0(preg_dir,subpopulations_names[a], "/GDPR/"), pattern="pregnancy_counts_ym_masked.csv")))
...
if(!is.null(preg_ym)){
  ...
  ggplotly(ggplot(preg_ym, aes(fill=meaning, y=percentage, x=year)) + 
...
```{r preg_ya, echo=F}
if (subpopulations_present=="No"){
  if(length(list.files(paste0(preg_dir,"GDPR/"), pattern="pregnancy_counts_ya_masked.csv"))>0){
preg_ya<-fread(paste0(preg_dir,"GDPR/", list.files(paste0(preg_dir,"GDPR/"), pattern="pregnancy_counts_ya_masked.csv")))
if(preg_ya[,.N]>0){
 if(preg_ya[,.N]>12000){
...
datatable(head(preg_ya,12000), options = list(scrollX=T), filter = "top")
 } else {
        datatable(preg_ya, options = list(scrollX=T), filter = "top")
 }
} else {
  preg_ya<-NULL
  print("This table cannot be produced based on the available data.")
  ...
  print("This table cannot be produced based on the available data.")
    ...
    if(length(list.files(paste0(preg_dir,subpopulations_names[a], "/GDPR/"), pattern="pregnancy_counts_ya_masked.csv"))>0){
  preg_ya<-fread(paste0(preg_dir,subpopulations_names[a], "/GDPR/", list.files(paste0(preg_dir,subpopulations_names[a], "/GDPR/"), pattern="pregnancy_counts_ya_masked.csv")))
  if (preg_ya[,.N]>0){
      if(preg_ya[,.N]>12000){  
    print("If the table contanins more than 12000 records it can not be displayed in the report. For the complete table, go to g_output/PREGNANCY/pregnancy_counts_ya_masked.csv")
      datatable(head(preg_ya,12000), options = list(scrollX=T), filter = "top")
...
if(!is.null(preg_ya)){
  ...
  ggplotly(ggplot(preg_ya, aes(fill=age_band, y=percentage, x=year)) + 
...
```{r preg_y, echo=F}
if (subpopulations_present=="No"){
  if(length(list.files(paste0(preg_dir,"GDPR/"), pattern="pregnancy_counts_y_masked.csv"))>0){
preg_y<-fread(paste0(preg_dir,"GDPR/", list.files(paste0(preg_dir,"GDPR/"), pattern="pregnancy_counts_y_masked.csv")))
if(preg_y[,.N]>0){
 if(preg_y[,.N]>12000){
...
datatable(head(preg_y,12000), options = list(scrollX=T), filter = "top")
 } else {
        datatable(preg_y, options = list(scrollX=T), filter = "top")
 }
} else {
  preg_y<-NULL
  print("This table cannot be produced based on the available data.")
  ...
  print("This table cannot be produced based on the available data.")
    ...
    if(length(list.files(paste0(preg_dir,subpopulations_names[a], "/GDPR/"), pattern="pregnancy_counts_y_masked.csv"))>0){
  preg_y<-fread(paste0(preg_dir,subpopulations_names[a], "/GDPR/", list.files(paste0(preg_dir,subpopulations_names[a], "/GDPR/"), pattern="pregnancy_counts_y_masked.csv")))
  if (preg_y[,.N]>0){
      if(preg_y[,.N]>12000){  
    print("If the table contanins more than 12000 records it can not be displayed in the report. For the complete table, go to g_output/PREGNANCY/pregnancy_counts_y_masked.csv")
      datatable(head(preg_y,12000), options = list(scrollX=T), filter = "top")
...
if(!is.null(preg_y)){
  ggplotly(ggplot(preg_y, aes(fill=event_definition, y=percentage, x=year)) + 
...
if (subpopulations_present=="No"){
  if(length(list.files(paste0(preg_dir,"GDPR/"), pattern="pregnancy_counts_ym_365_masked.csv"))>0){
preg_ym_365<-fread(paste0(preg_dir,"GDPR/", list.files(paste0(preg_dir,"GDPR/"), pattern="pregnancy_counts_ym_365_masked.csv")))
if(preg_ym_365[,.N]>0){
...
datatable(head(preg_ym_365,12000), options = list(scrollX=T), filter = "top")
 } else {
        datatable(preg_ym_365, options = list(scrollX=T), filter = "top")
 }
} else {
  preg_ym_365<-NULL
  print("This table cannot be produced based on the available data.")
  ...
  print("This table cannot be produced based on the available data.")
    ...
    if(length(list.files(paste0(preg_dir,subpopulations_names[a], "/GDPR/"), pattern="pregnancy_counts_ym_365_masked.csv"))>0){
  preg_ym_365<-fread(paste0(preg_dir,subpopulations_names[a], "/GDPR/", list.files(paste0(preg_dir,subpopulations_names[a], "/GDPR/"), pattern="pregnancy_counts_ym_365_masked.csv")))
  if (preg_ym_365[,.N]>0){
      if(preg_ym_365[,.N]>12000){  
    print("If the table contanins more than 12000 records it can not be displayed in the report. For the complete table, go to g_output/PREGNANCY/pregnancy_counts_ym_365_masked.csv")
      datatable(head(preg_ym_365,12000), options = list(scrollX=T), filter = "top")
...
if(!is.null(preg_ym_365)){
  ...
  ggplotly(ggplot(preg_ym_365, aes(fill=meaning, y=percentage, x=year)) + 
   geom_line(aes(color=meaning)) +
  geom_point(aes(color=meaning)) + 
    facet_wrap( ~ event_definition) +
    ggtitle("Pregnancy counts by meaning, year and event definition") +
...
datatable(calculations_pregnancy, filter = "top")
```
