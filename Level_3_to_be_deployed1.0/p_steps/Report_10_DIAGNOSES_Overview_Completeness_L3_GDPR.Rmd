...
subtitle: "GDPR: Overview and completeness of the Diagnoses study population"
output: 
  html_document:
    theme: spacelab
    toc: true
...
```{r echo=F}
if (subpopulations_present=="No"){
flowchart_diagnoses<-fread(paste0(diag_dir,"GDPR/", list.files(paste0(diag_dir,"GDPR/"),"diagnoses_flowchart_masked.csv")))
datatable(flowchart_diagnoses, options = list(scrollX=T))
} else {
flowchart_diagnoses<-fread(paste0(diag_dir,subpopulations_names[a], "/GDPR/",list.files(paste0(diag_dir,subpopulations_names[a], "/GDPR/"),paste0(subpopulations_names[a],"_diagnoses_flowchart_masked.csv"))))
datatable(flowchart_diagnoses, options = list(scrollX=T))
}
```
...
```{r echo=F}
if (subpopulations_present=="No"){
description_diagnoses<-fread(paste0(diag_dir,"GDPR/", list.files(paste0(diag_dir, "GDPR/"),"diagnoses_description_masked.csv")))
datatable(description_diagnoses, options = list(scrollX=T))
} else {
  description_diagnoses<-fread(paste0(diag_dir,subpopulations_names[a], "/GDPR/",
                                list.files(paste0(diag_dir,subpopulations_names[a], "/GDPR/"),paste0(subpopulations_names[a],"_diagnoses_description_masked.csv"))))
datatable(description_diagnoses, options = list(scrollX=T))
}
```
...
```{r echo=F}
if (subpopulations_present=="No"){
  if(length(list.files(paste0(diag_dir,"GDPR/"), pattern="diagnoses_completeness_masked.csv"))>0){
tab20_diag<-fread(paste0(diag_dir,"GDPR/",list.files(paste0(diag_dir,"GDPR/"), "diagnoses_completeness_masked.csv")))
if(tab20_diag[,.N]>0){
datatable(tab20_diag, options = list(scrollX=T), filter = "top")
  } else {
    tab20_diag<-NULL
  ...
  print("This table cannot be produced based on the available data.")
    ...
    if(length(list.files(paste0(diag_dir,subpopulations_names[a], "/GDPR/"), pattern="diagnoses_completeness_masked.csv"))>0){
    tab20_diag<-fread(paste0(diag_dir,subpopulations_names[a], "/GDPR/",list.files(paste0(diag_dir,subpopulations_names[a], "/GDPR/"), pattern="diagnoses_completeness_masked.csv")))
    if(tab20_diag[,.N]>0){
datatable(tab20_diag, options = list(scrollX=T), filter = "top")
    } else {
      tab20_diag<-NULL
...
datatable(codes, filter = 'top', options = list(paging = TRUE, searching = TRUE, sorting = TRUE, scrollX=TRUE))
```
