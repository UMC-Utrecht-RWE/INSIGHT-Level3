...
subtitle: "Overview and completeness of the MEDICINES table"
output: 
  html_document:
    theme: spacelab
    toc: true
...
```{r flowchart, echo=F}
if (subpopulations_present=="No"){
flowchart_med<-fread(paste0(med_dir,"Masked/", list.files(paste0(med_dir,"Masked/"),"medicines_flowchart_masked.csv")))
datatable(flowchart_med, options = list(scrollX=T))
} else {
flowchart_med<-fread(paste0(med_dir,subpopulations_names[a], "/Masked/",list.files(paste0(med_dir,subpopulations_names[a], "/Masked/"),paste0(subpopulations_names[a],"_medicines_flowchart_masked.csv"))))
datatable(flowchart_med, options = list(scrollX=T))
}
```
...
if (subpopulations_present=="No"){
description_med<-fread(paste0(med_dir,"Masked/", list.files(paste0(med_dir, "Masked/"),"medicines_description_masked.csv")))
datatable(description_med, options = list(scrollX=T))
} else {
  description_med<-fread(paste0(med_dir,subpopulations_names[a], "/Masked/",
                                list.files(paste0(med_dir,subpopulations_names[a], "/Masked/"),paste0(subpopulations_names[a],"_medicines_description_masked.csv"))))
datatable(description_med, options = list(scrollX=T))
}
```
...
if (subpopulations_present=="No"){
  if(length(list.files(paste0(med_dir,"Masked/"), pattern="medicines_completeness_masked.csv"))>0){
tab15_med<-fread(paste0(med_dir,"Masked/", list.files(paste0(med_dir,"Masked/"), pattern="medicines_completeness_masked.csv")))
if (tab15_med[,.N]>0){
datatable(tab15_med, options = list(scrollX=T))
  ...
  tab15_med<-NULL
    ...
    if(length(list.files(paste0(med_dir,subpopulations_names[a], "/Masked/"), pattern="medicines_completeness_masked.csv"))>0){
  tab15_med<-fread(paste0(med_dir,subpopulations_names[a], "/Masked/", list.files(paste0(med_dir,subpopulations_names[a], "/Masked/"), pattern="medicines_completeness_masked.csv")))
  if (tab15_med[,.N]>0){
datatable(tab15_med, options = list(scrollX=T))
  } else {
    tab15_med<-NULL
