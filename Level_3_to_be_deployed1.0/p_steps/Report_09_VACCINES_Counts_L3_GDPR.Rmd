...
setcolorder(tab10_vx,c("vaccine_indicator","year","meaning","percentage", "count","total", "data_access_provider", "data_source"))
if(tab10_vx[,.N]>0){
 if(tab10_vx[,.N]>12000){
...
datatable(head(tab10_vx,12000), options = list(scrollX=T), filter = "top")
 } else {
        datatable(tab10_vx, options = list(scrollX=T), filter = "top")
 }
} else {
  tab10_vx<-NULL
...
ggplotly(ggplot(tab10_graph_vx, aes(x = year, y = percentage, group=vaccine_indicator)) +
...
tab12_vx<-merge.data.table(tab12_vx, rec, by="no_records", all.x=T, allow.cartesian = T)
setcolorder(tab12_vx,c("sex","year","atc_4","no_records","no_users","median_records","mean_records","label_records","meaning","atc_1","atc_3"))
if(tab12_vx[,.N]>0){
  if(tab12_vx[,.N]>12000){
    ...
    datatable(head(tab12_vx,12000), options = list(scrollX=T), filter = "top")
  } else {
datatable(tab12_vx, options = list(scrollX=T), filter = "top")
  }
} else {
  tab12_vx<-NULL
    print("This table cannot be produced based on the available data.")
...
if(var_to_keep=="vx_atc"){
if(!is.null(tab12_vx)){
tab12_graph_vx<-tab12_vx[,.(meaning,sex,year,atc_4,no_records,no_users, median_records, mean_records,label_records)]
}
...
for(i in 1:length(unique(tab12_graph_vx[,meaning]))){
  tab12_graph_vx<-tab12_graph_vx[,label_records:=as.numeric(label_records)]
fig.tab12_graph_vx[[i]]<-ggplotly(ggplot(tab12_graph_vx[meaning==unique(tab12_graph_vx[,meaning])[i]], aes(x = year, y = label_records, group=atc_4)) +
...
rec<-records_categories[,c("range", "label_records")]
setnames(rec, "range", "no_records")
tab13_vx<-merge.data.table(tab13_vx, rec, by="no_records", all.x=T, allow.cartesian = T)
setcolorder(tab13_vx,c("sex","year","atc_7", "no_records","no_users", "median_records","mean_records","label_records","meaning", "data_access_provider", "data_source"))
...
if(tab13_vx[,.N]>0){
  if(tab13_vx[,.N]>12000){
    ...
    datatable(head(tab13_vx,12000), options = list(scrollX=T), filter = "top")
  } else {
  datatable(tab13_vx, options = list(scrollX=T), filter = "top")
  }
} else {
  tab13_vx<-NULL
    print("This table cannot be produced based on the available data.")
...
if(!is.null(tab13_vx)){
tab13_vx_graph<-tab13_vx[,.(sex,year,atc_7,label_records)]
tab13_vx_graph<-tab13_vx_graph[order(year,label_records)]
tab13_vx_graph[,year:=as.factor(year)]
}

if(!is.null(tab13_vx_graph)){
if (tab13_vx_graph[,.N] != 0){
ggplotly(ggplot(tab13_vx_graph, aes(x = year, y = label_records, group=atc_7)) +
