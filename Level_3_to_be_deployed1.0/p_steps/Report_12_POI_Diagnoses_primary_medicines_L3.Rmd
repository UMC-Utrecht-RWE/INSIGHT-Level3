...
subtitle: "Medication use after a diagnosis"
output: 
  html_document:
    theme: spacelab
    toc: true
...
if (subpopulations_present=="No"){
  if(sum(grepl(list.files(poi_dir), pattern="diag_secondary_rec_removed.csv"))>0){
res<-fread(paste0(poi_dir,list.files(poi_dir,"diag_secondary_rec_removed.csv")))
if(res[,.N]>0){
datatable(res, options = list(scrollX=T))
  ...
  print("This table cannot be produced based on the available data.")
    ...
    if(sum(grepl(list.files(paste0(poi_dir,subpopulations_names[a], "/Masked/")), pattern="diag_secondary_rec_removed.csv"))>0){
...
datatable(res, options = list(scrollX=T))
  ...
  print("This table cannot be produced based on the available data.")
    ...
    if(sum(grepl(list.files(paste0(poi_dir,subpopulations_names[a], "/Masked/")), pattern="diag_summary_smy_masked"))>0){
...
datatable(res, options = list(scrollX=T))
  ...
  print("This table cannot be produced based on the available data.")
    ...
    if(sum(grepl(list.files(paste0(poi_dir,subpopulations_names[a], "/Masked/")), pattern="diag_summary_sm_masked.csv"))>0){
      fl<-paste0(poi_dir,subpopulations_names[a], "/Masked/")
...
replaced with a count of 3. Then the approximate prevalence rates will
be calculated as number of users with a diagnosis divided by number of
subjects with a diagnosis and multiplied by 100.</mark>

```{r , echo=FALSE, warning=FALSE}
if(!is.null(res_4)){
rec_graph<-as.data.table(res_4[no_users=="<5", no_users:= 3])
...
```{r , echo=FALSE, warning=FALSE}
if(!is.null(res_5)){
rec_graph<-as.data.table(res_5[no_users=="<5", no_users:= 3])
rec_graph<-rec_graph[,no_users:=as.numeric(no_users)]
rec_graph<-rec_graph[no_subjects=="<5", no_subjects:= 3]
rec_graph<-rec_graph[,no_subjects:=as.numeric(no_subjects)]
rec_graph<-rec_graph[,year:=as.factor(year)]
...
<mark style="background-color: #d4e5d2">replaced with a count of 3. Then the approximate percentage will be
calculated as number of records divided by total number of records and
multiplied by 100. When the number of records is zero it is not shown in
the graph. See graph
`Medication use prevalence by event definition, sex, ATC code and year after a diagnosis`
...
```{r , echo=FALSE, warning=FALSE}
if(!is.null(res_7)){
rec_graph<-as.data.table(res_7[no_records=="<5", no_records:= 3])
rec_graph<-rec_graph[,no_records:=as.numeric(no_records)]
rec_graph<-rec_graph[total_records=="<5", total_records:= 3]
rec_graph<-rec_graph[,total_records:=as.numeric(total_records)]
rec_graph<-rec_graph[,year:=as.factor(year)]
...
```{r , echo=FALSE, warning=FALSE}
if(!is.null(res_8)){
rec_graph<-as.data.table(res_8[no_records=="<5", no_records:= 3])
rec_graph<-rec_graph[,no_records:=as.numeric(no_records)]
rec_graph<-rec_graph[total_records=="<5", total_records:= 3]
rec_graph<-rec_graph[,total_records:=as.numeric(total_records)]
rec_graph<-rec_graph[,year:=as.factor(year)]
