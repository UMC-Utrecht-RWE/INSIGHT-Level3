...
subtitle: "GDPR: Vaccines administrations after a diagnosis"
output: 
  html_document:
    theme: spacelab
    toc: true
...
datatable(res, options = list(scrollX=T), filter="top")
  ...
  print("This table cannot be produced based on the available data or there were no exclusions.")
    ...
    if(length(list.files(paste0(poi_dir,subpopulations_names[a], "/GDPR/"), pattern="diag_secondary_rec_removed.csv"))>0){
...
```{r echo=F, warning=FALSE}
if (subpopulations_present=="No"){
  if(length(list.files(paste0(poi_dir,"GDPR/"), pattern="diag_vacc_prior_removed"))>0){
fl<-paste0(poi_dir,"GDPR/")
res_vacc<-fread(paste0(fl,list.files(fl,"diag_vacc_prior_removed.csv")))
if(res_vacc[,.N]>0){
datatable(res_vacc, options = list(scrollX=T),filter="top")
  } else {
    res_vacc<-NULL
  print("This table cannot be produced based on the available data or there were no exclusions.")
  }
  ...
  print("This table cannot be produced based on the available data or there were no exclusions.")
    ...
    if(length(list.files(paste0(poi_dir,"EVENTS_VACCINES/",subpopulations_names[a], "/GDPR/"), pattern="diag_vacc_prior_removed.csv"))>0){
...
```{r echo=F, warning=FALSE}
if (subpopulations_present=="No"){
  if(length(list.files(paste0(poi_dir,"GDPR/"), pattern="diag_vacc_dup_removed.csv"))>0){
fl<-paste0(poi_dir,"GDPR/")
res_vacc_2<-fread(paste0(fl,list.files(fl,"diag_vacc_dup_removed.csv")))
if(res_vacc_2[,.N]>0){
datatable(res_vacc_2, options = list(scrollX=T), filter="top")
  } else {
    res_vacc_2<-NULL
  print("This table cannot be produced based on the available data or there were no exclusions.")
  }
  ...
  print("This table cannot be produced based on the available data or there were no exclusions.")
    ...
    if(length(list.files(paste0(poi_dir,"EVENTS_VACCINES/",subpopulations_names[a], "/GDPR/"), pattern="diag_vacc_dup_removed.csv"))>0){
      fl<-paste0(poi_dir,"EVENTS_VACCINES/",subpopulations_names[a],"/")
...
datatable(res, options = list(scrollX=T), filter = "top")
  ...
  print("This table cannot be produced based on the available data.")
    ...
    if(length(list.files(paste0(poi_dir,subpopulations_names[a], "/GDPR/"), pattern="diag_summary_smy_masked"))>0){
...
res<-fread(paste0(poi_dir,"GDPR/",list.files(paste0(poi_dir,"GDPR/"),"diag_summary_sm_masked.csv")))
if(res[,.N]>0){
datatable(res, options = list(scrollX=T), filter="top")
  } else {
    res<-NULL
  ...
  print("This table cannot be produced based on the available data.")
    ...
    if(length(list.files(paste0(poi_dir,subpopulations_names[a], "/GDPR/"), pattern="diag_summary_sm_masked.csv"))>0){
      fl<-paste0(poi_dir,subpopulations_names[a], "/GDPR/")
...
```{r echo=F, warning=FALSE}
if (subpopulations_present=="No"){
  if(length(list.files(paste0(poi_dir,"GDPR/"), pattern="diag_vacc_prevalence_ys_masked.csv"))>0){
fl<-paste0(poi_dir,"GDPR/")
res_9<-fread(paste0(fl,list.files(fl,"diag_vacc_prevalence_ys_masked.csv")))
if(res_9[,.N]>0){
datatable(res_9, options = list(scrollX=T), filter="top")
  } else {
    res_9<-NULL
  print("This table cannot be produced based on the available data.")
  }
  ...
  print("This table cannot be produced based on the available data.")
    ...
    if(length(list.files(paste0(poi_dir,"EVENTS_VACCINES/",subpopulations_names[a], "/GDPR/"), pattern="diag_vacc_prevalence_ys_masked.csv"))>0){
      fl<-paste0(poi_dir,"EVENTS_VACCINES/",subpopulations_names[a], "/GDPR/")
...
if(!is.null(res_9)){
rec_graph<-res_9[,.(event_definition, sex, year, vaccine_indicator, prevalence_vaccine_exposure)]
...
fig.1[[i]]<-ggplotly(ggplot(rec_graph[event_definition==unique(rec_graph[,event_definition])[i]], aes(x = year, y = prevalence_vaccine_exposure, group=event_definition)) +
                           ...
                           geom_point(aes(color=vaccine_indicator)) + 
                               ggtitle(paste0("Vaccine exposure prevalence:", unique(rec_graph[,event_definition])[i])) + 
...
```{r echo=F, warning=FALSE}
if (subpopulations_present=="No"){
  if(length(list.files(paste0(poi_dir,"GDPR/"), pattern="diag_vacc_prevalence_y_masked.csv"))>0){
fl<-paste0(poi_dir,"GDPR/")
res_10<-fread(paste0(fl,list.files(fl,"diag_vacc_prevalence_y_masked.csv")))
if(res_10[,.N]>0){
datatable(res_10, options = list(scrollX=T), filter="top")
  } else {
    res_10<-NULL
  print("This table cannot be produced based on the available data.")
  }
  ...
  print("This table cannot be produced based on the available data.")
    ...
    if(length(list.files(paste0(poi_dir,"EVENTS_VACCINES/",subpopulations_names[a], "/GDPR/"), pattern="diag_vacc_prevalence_y_masked.csv"))>0){
      fl<-paste0(poi_dir,"EVENTS_VACCINES/",subpopulations_names[a], "/GDPR/")
...
```{r , echo=FALSE, warning=FALSE}
if(!is.null(res_10)){
rec_graph<-as.data.table(res_10[,.(event_definition,year,vaccine_indicator,prevalence_vaccine_exposure)])
rec_graph<-rec_graph[,year:=as.factor(year)]
#Set columns to the right type
rec_graph[,prevalence_vaccine_exposure:=as.numeric(as.character(prevalence_vaccine_exposure))]
...
```{r echo=F, warning=FALSE}
if (subpopulations_present=="No"){
  if(length(list.files(paste0(poi_dir,"GDPR/"), pattern="diag_vacc_counts_ys_masked.csv"))>0){
fl<-paste0(poi_dir,"GDPR/")
res_11<-fread(paste0(fl,list.files(fl,"diag_vacc_counts_ys_masked.csv")))
if(res_11[,.N]>0){
datatable(res_11, options = list(scrollX=T), filter="top")
  } else {
    res_11<-NULL
  print("This table cannot be produced based on the available data.")
  }
  ...
  print("This table cannot be produced based on the available data.")
    ...
    if(length(list.files(paste0(poi_dir,"EVENTS_VACCINES/",subpopulations_names[a], "/GDPR/"), pattern="diag_vacc_counts_ys_masked.csv"))>0){
      fl<-paste0(poi_dir,"EVENTS_VACCINES/",subpopulations_names[a], "/GDPR/")
...
```{r , echo=FALSE, warning=FALSE}
if(!is.null(res_11)){
rec_graph<-as.data.table(res_11[,.(event_definition, sex, year, vaccine_indicator, percentage)])
rec_graph<-rec_graph[,year:=as.factor(year)]
#Set columns to the right type
...
fig.1[[i]]<-ggplotly(ggplot(rec_graph[event_definition==unique(rec_graph[,event_definition])[i]], aes(x = year, y = percentage, group=event_definition)) +
                           geom_line(aes(color=vaccine_indicator)) +
                           geom_point(aes(color=vaccine_indicator)) + 
                               ggtitle(paste0("Vaccine exposure:", unique(rec_graph[,event_definition])[i])) + 
                               xlab("Year") +
                               ylab("Percentage")+
                       facet_wrap(~sex, scales = "fixed")+
                             theme_classic() +
...
```{r echo=F, warning=FALSE}
if (subpopulations_present=="No"){
  if(length(list.files(paste0(poi_dir,"GDPR/"), pattern="diag_vacc_counts_ym_masked.csv"))>0){
fl<-paste0(poi_dir,"GDPR/")
res_12<-fread(paste0(fl,list.files(fl,"diag_vacc_counts_ym_masked.csv")))
if(res_12[,.N]>0){
datatable(res_12, options = list(scrollX=T), filter="top")
  } else {
    res_12<-NULL
  print("This table cannot be produced based on the available data.")
  }
  ...
  print("This table cannot be produced based on the available data.")
    ...
    if(length(list.files(paste0(poi_dir,"EVENTS_VACCINES/",subpopulations_names[a], "/GDPR/"), pattern="diag_vacc_counts_ym_masked.csv"))>0){
      res_12<-paste0(poi_dir,"EVENTS_VACCINES/",subpopulations_names[a], "/GDPR/")
...
```{r , echo=FALSE, warning=FALSE}
if(!is.null(res_12)){
rec_graph<-as.data.table(res_12[,.(event_definition, meaning, year, vaccine_indicator, percentage)])
rec_graph<-rec_graph[,year:=as.factor(year)]
#Set columns to the right type
rec_graph[,percentage:=as.numeric(as.character(percentage))]
...
```{r echo=F, warning=FALSE}
if (subpopulations_present=="No"){
  if(length(list.files(paste0(poi_dir,"GDPR/"), pattern="diag_vacc_counts_y_masked.csv"))>0){
fl<-paste0(poi_dir,"GDPR/")
res_13<-fread(paste0(fl,list.files(fl,"diag_vacc_counts_y_masked.csv")))
if(res_13[,.N]>0){
datatable(res_13, options = list(scrollX=T), filter="top")
  } else {
    res_13<-NULL
  print("This table cannot be produced based on the available data.")
  }
  ...
  print("This table cannot be produced based on the available data.")
    ...
    if(length(list.files(paste0(poi_dir,"EVENTS_VACCINES/",subpopulations_names[a], "/GDPR/"), pattern="diag_vacc_counts_y_masked.csv"))>0){
      res_13<-paste0(poi_dir,"EVENTS_VACCINES/",subpopulations_names[a], "/GDPR/")
...
```{r , echo=FALSE, warning=FALSE}
if(!is.null(res_13)){
rec_graph<-as.data.table(res_13[,.(event_definition, year, vaccine_indicator, percentage)])
rec_graph<-rec_graph[,year:=as.factor(year)]
#Set columns to the right type
rec_graph<-as.data.table(rec_graph)
rec_graph[,percentage:=as.numeric(as.character(percentage))]
