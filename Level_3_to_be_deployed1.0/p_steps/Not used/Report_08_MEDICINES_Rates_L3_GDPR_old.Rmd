...
subtitle: "GDPR: Rates of medicines use in the study population."
output: 
  html_document:
    theme: spacelab
    toc: true
...
```{r rates_year, echo=F}
if (subpopulations_present=="No"){
  if(length(list.files(paste0(med_dir,"GDPR/"), pattern="medicines_rates_year_atc_masked.csv"))>0){
tab15_med<-fread(paste0(med_dir,"GDPR/", list.files(paste0(med_dir,"GDPR/"), pattern="medicines_rates_year_atc_masked.csv")))
if (tab15_med[,.N]>0){
...
datatable(tab15_med, options = list(scrollX=T))
  ...
  tab15_med<-NULL
    ...
    if(length(list.files(paste0(med_dir,subpopulations_names[a], "/GDPR/"), pattern="medicines_rates_year_atc_masked.csv"))>0){
  tab15_med<-fread(paste0(med_dir,subpopulations_names[a], "/GDPR/", list.files(paste0(med_dir,subpopulations_names[a], "/GDPR/"), pattern="medicines_rates_year_atc_masked.csv")))
  if (tab15_med[,.N]>0){
   if (tab15_med[,.N]>12000){   
            print("If the table contanins more than 12000 records it can not be displayed in the report. For the complete table, go to g_output/MEDICINES/medicines_rates_year_atc_masked.csv")
        datatable(head(tab15_med,12000), options = list(scrollX=T))
...
#### Graph: Rates of medicines exposure in females by year and ATC code
...
tab15_graph_med<-tab15_graph_med[no_users=="<5", no_users:= 3]
tab15_graph_med<-tab15_graph_med[,no_users:=as.numeric(no_users)]
tab15_graph_med<-tab15_graph_med[no_subjects=="<5", no_subjects:= 3]
tab15_graph_med<-tab15_graph_med[,no_subjects:=as.numeric(no_subjects)]
...
tab15_graph_med<-tab15_graph_med[users_per_100_py=="N/A", users_per_100_py:= round((no_users/person_years)*100,2)]
tab15_graph_med<-tab15_graph_med[subjects_per_100_py=="N/A", subjects_per_100_py:= round((no_subjects/person_years)*100,2)]
} else {
 tab15_graph_med<-NULL 
}

if(!is.null(tab15_graph_med)){
  if(tab15_graph_med[sex=="F",.N]>0){
...
ggplotly(ggplot(tab15_graph_med[sex=="F"], aes(x = year, y = medicines_per_100_py, group=truncated_atc_code)) +
...
#### Graph: Rates of medicines exposure in males by year and ATC code

```{r tab15_graph_medicines_m, echo=F}
if(!is.null(tab15_graph_med)){
  if(tab15_graph_med[sex=="M",.N]>0){
     tab15_graph_med[,medicines_per_100_py:=as.numeric(medicines_per_100_py)]
    tab15_graph_med[,medicines_per_100_py:=round(medicines_per_100_py,1)]
ggplotly(ggplot(tab15_graph_med[sex=="M"], aes(x = year, y = medicines_per_100_py, group=truncated_atc_code)) +
                           geom_line(aes(color=truncated_atc_code)) +
                           geom_point(aes(color=truncated_atc_code)) + 
                               ggtitle("Rate of medicine exposure in the males") +
                               xlab("Year") +
                               ylab("Rate per 100 person-years")+
                             theme_classic() +
                             guides(shape = guide_legend(override.aes = list(size = 0.3))) +
...
if(!is.null(tab15_graph_med)){
ggplotly(ggplot(tab15_graph_med, aes(x = year, y = users_per_100_py, group=truncated_atc_code)) +
                           geom_line(aes(color=truncated_atc_code)) +
                           geom_point(aes(color=truncated_atc_code)) + 
...
if(!is.null(tab15_graph_med)){
ggplotly(ggplot(tab15_graph_med, aes(x = year, y = no_subjects, group=truncated_atc_code)) +
                           geom_line(aes(color=truncated_atc_code)) +
                           geom_point(aes(color=truncated_atc_code)) + 
            facet_wrap(~ sex, ncol=2, scales = "fixed") +
...
if (subpopulations_present=="No"){
  if(length(list.files(paste0(med_dir,"GDPR/"), pattern="medicines_rates_year_age_atc_masked.csv"))>0){
tab16_med<-fread(paste0(med_dir,"GDPR/", list.files(paste0(med_dir,"GDPR/"), pattern="medicines_rates_year_age_atc_masked.csv")))
if (tab16_med[,.N]>0){
...
datatable(tab16_med, options = list(scrollX=T))  
    ...
    if(length(list.files(paste0(med_dir,subpopulations_names[a], "/GDPR/"), pattern="medicines_rates_by_year_age_atc_masked.csv"))>0){
  tab16_med<-fread(paste0(med_dir,subpopulations_names[a], "/GDPR/", list.files(paste0(med_dir,subpopulations_names[a], "/GDPR/"), pattern="medicines_rates_by_year_age_atc_masked.csv")))
  if (tab16_med[,.N]>0){
    if(tab16_med[,.N]>12000){
      print("If the table contanins more than 12000 records it can not be displayed in the report. For the complete table, go to g_output/MEDICINES/medicines_rates_year_age_atc_masked.csv")
datatable(head(tab16_med,12000), options = list(scrollX=T))
...
tab16_graph_med<-tab16_graph_med[no_users=="<5", no_users:= 3]
tab16_graph_med<-tab16_graph_med[,no_users:=as.numeric(no_users)]
tab16_graph_med<-tab16_graph_med[no_subjects=="<5", no_subjects:= 3]
tab16_graph_med<-tab16_graph_med[,no_subjects:=as.numeric(no_subjects)]
...
tab16_graph_med<-tab16_graph_med[users_per_100_py=="N/A", users_per_100_py:= round((no_users/person_years)*100,2)]
tab16_graph_med<-tab16_graph_med[subjects_per_100_py=="N/A", subjects_per_100_py:= round((no_subjects/person_years)*100,2)]
...
fig.tab16_med_med_f<-vector(mode="list", length=length(unique(tab16_graph_med[sex=="F",age_band])))
for(i in 1:length(unique(tab16_graph_med[sex=="F",age_band]))){
fig.tab16_med_med_f[[i]]<-ggplotly(ggplot(tab16_graph_med[sex=="F"][age_band==unique(tab16_graph_med[,age_band])[i]], aes(x = year, y = medicines_per_100_py, group=truncated_atc_code)) +
                           geom_line(aes(color=truncated_atc_code)) +
                           geom_point(aes(color=truncated_atc_code)) + 
                               ggtitle(paste0("Rate of medicine exposure in females, age band:", unique(tab16_graph_med[sex=="F",age_band])[i])) + 
                               xlab("Year") +
                               ylab("Rate per 100 person-years")+
                             theme_classic() +
                             guides(shape = guide_legend(override.aes = list(size = 0.3))) +
...
#### Graph: Rates of medicines exposure in males by year, age and ATC code

```{r tab16_graph_med_m, echo=FALSE, warning=FALSE}
if(!is.null(tab16_graph_med)){
  if(tab16_graph_med[sex=="M",.N]>0){
    tab16_graph_med[,medicines_per_100_py:=as.numeric(medicines_per_100_py)]
    tab16_graph_med[,medicines_per_100_py:=round(medicines_per_100_py,1)]
fig.tab16_med_med_m<-vector(mode="list", length=length(unique(tab16_graph_med[sex=="M",age_band])))
for(i in 1:length(unique(tab16_graph_med[sex=="M",age_band]))){
fig.tab16_med_med_m[[i]]<-ggplotly(ggplot(tab16_graph_med[sex=="M"][age_band==unique(tab16_graph_med[,age_band])[i]], aes(x = year, y = medicines_per_100_py, group=truncated_atc_code)) +
                           geom_line(aes(color=truncated_atc_code)) +
                           geom_point(aes(color=truncated_atc_code)) + 
                               ggtitle(paste0("Rate of medicine exposure in males, age band:", unique(tab16_graph_med[sex=="M",age_band])[i])) + 
                               xlab("Year") +
                               ylab("Rate per 100 person-years")+
                             theme_classic() +
                             guides(shape = guide_legend(override.aes = list(size = 0.3))) +
...
if(!is.null(tab16_graph_med)){
fig.tab16_user_med<-vector(mode="list", length=length(unique(tab16_graph_med[,age_band])))
for(i in 1:length(unique(tab16_graph_med[,age_band]))){
fig.tab16_user_med[[i]]<-ggplotly(ggplot(tab16_graph_med[age_band==unique(tab16_graph_med[,age_band])[i]], aes(x = year, y = users_per_100_py, group=truncated_atc_code)) +
                           geom_line(aes(color=truncated_atc_code)) +
                           geom_point(aes(color=truncated_atc_code)) + 
                               ggtitle(paste0("Rate of users, age band:", unique(tab16_graph_med[,age_band])[i])) + 
                               xlab("Year") +
                               ylab("Rate per 100 person-years")+
                             theme_classic() +
                             guides(shape = guide_legend(override.aes = list(size = 0.3))) +
...
if(!is.null(tab16_graph_med)){
fig.tab16_subjects_med<-vector(mode="list", length=length(unique(tab16_graph_med[,age_band])))
for(i in 1:length(unique(tab16_graph_med[,age_band]))){
fig.tab16_subjects_med[[i]]<-ggplotly(ggplot(tab16_graph_med[age_band==unique(tab16_graph_med[,age_band])[i]], aes(x = year, y = subjects_per_100_py, group=truncated_atc_code)) +
                           geom_line(aes(color=truncated_atc_code)) +
                           geom_point(aes(color=truncated_atc_code)) + 
                               ggtitle(paste0("Rate of subjects, age band:", unique(tab16_graph_med[,age_band])[i])) + 
                               xlab("Year") +
                               ylab("Rate per 100 person-years")+
                             theme_classic() +
                             guides(shape = guide_legend(override.aes = list(size = 0.3))) +
...
if(!is.null(tab16_graph_med)){
htmltools::tagList(list(fig.tab16_subjects_med))
} else {
  print("This table cannot be produced based on the available data.") 
}
```
