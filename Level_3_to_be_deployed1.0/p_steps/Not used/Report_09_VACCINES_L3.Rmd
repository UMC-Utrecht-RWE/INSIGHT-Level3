...
if(!is.null(tab10)){
tab10_graph<-tab10[count=="<5", count:= 3]
...
if(tab12[,.N]>0){
setcolorder(tab12,c("atc_code_4", "no_records", "no_male_users", "median_rx_male_users","mean_rx_male_users", "no_female_users", "median_rx_female_users","mean_rx_female_users", "meaning", "year", "atc_code_3","atc_code_1", "data_access_provider", "data_source"))
  if(tab12[,.N]>12000){
...
if(!is.null(tab12)){
tab12_graph<-tab12[,.(meaning,year,atc_code_4,no_records,no_male_users, no_female_users, median_rx_male_users, median_rx_female_users, mean_rx_male_users, mean_rx_female_users)]
tab12_graph<-tab12_graph[no_records=="<5",no_records:=3]
tab12_graph<-tab12_graph[no_male_users=="<5",no_male_users:=3]
tab12_graph<-tab12_graph[no_female_users=="<5",no_female_users:=3]
...
tab12_graph_users<-as.data.table(tab12_graph_users)
...
### Median number of administrations  
...
tab12_graph_median[,meaning:=as.character(meaning)][,year:=as.character(year)][,median_rx_male_users:=as.numeric(median_rx_male_users)][,median_rx_female_users:=as.numeric(median_rx_female_users)]
tab12_graph_median<-melt(tab12_graph_median, id.vars = c("meaning", "year","atc_code_4"), variable.name = "sex")
setnames(tab12_graph_median, "value", "median_administrations")
tab12_graph_median<-as.data.table(tab12_graph_median)
tab12_graph_median<-tab12_graph_median[,median_administrations:=as.numeric(median_administrations)]
...
fig.tab12_median[[i]]<-ggplotly(ggplot(tab12_graph_median[meaning==unique(tab12_graph_median[,meaning])[i]], aes(x = year, y = median_administrations, group=atc_code_4)) +
                           geom_line(aes(color=atc_code_4)) +
                           geom_point(aes(color=atc_code_4)) + 
                             facet_wrap(~ sex, ncol=2, scales = "fixed") +
                               ggtitle(paste0("Median number of administrations by ATC code, year and sex, meaning:",unique(tab12_graph_median[,meaning])[i])) + 
                               xlab("Year") +
                               ylab("Count")+
                             theme_classic() +
                             guides(shape = guide_legend(override.aes = list(size = 0.3))) +
...
if(!is.null(tab12_graph)){
tab12_graph_mean<-tab12_graph[meaning!="All",.(meaning,year,atc_code_4,mean_rx_male_users,mean_rx_female_users)]
tab12_graph_mean<-melt(tab12_graph_mean, id.vars = c("meaning", "year","atc_code_4"), variable.name = "sex")
setnames(tab12_graph_mean, "value", "mean_administrations")
tab12_graph_mean<-tab12_graph_mean[,mean_administrations:=as.numeric(mean_administrations)]
tab12_graph_mean<-tab12_graph_mean[sex=="mean_rx_male_users", sex:="Male"][sex=="mean_rx_female_users", sex:="Female"]
fig.tab12_mean<-vector(mode="list", length=length(unique(tab12_graph_mean[,meaning])))
for(i in 1:length(unique(tab12_graph_mean[,meaning]))) {
fig.tab12_mean[[i]]<-ggplotly(ggplot(tab12_graph_mean[meaning==unique(tab12_graph_mean[,meaning])[i]], aes(x = year, y = mean_administrations, group=atc_code_4)) +
                           geom_line(aes(color=atc_code_4)) +
                           geom_point(aes(color=atc_code_4)) + 
                            facet_wrap(~ sex, ncol=2, scales = "fixed") +
                               ggtitle(paste0("Mean number of administrations by ATC code, year and sex, meaning:",unique(tab12_graph_mean[,meaning])[i])) + 
                               xlab("Year") +
                               ylab("Count")+
                             theme_classic() +
                             guides(shape = guide_legend(override.aes = list(size = 0.3))) +
...
#Move the x axis title down so it doesn't overlap with the labels
for (i in 1: length(fig.tab12_mean)){
fig.tab12_mean[[i]][['x']][['layout']][['annotations']][[1]][['y']] <--0.15
}
}
...
if(!is.null(tab12_graph)){
htmltools::tagList(list(fig.tab12_mean))
} else {
  print("This table cannot be produced based on the available data.") 
}
```
...
setcolorder(tab13,c("atc_code_7", "no_records", "no_male_users", "median_rx_male_users","mean_rx_male_users", "no_female_users", "median_rx_female_users","mean_rx_female_users", "meaning", "year", "atc_code_3", "data_access_provider", "data_source"))
...
if(tab11[,.N]>0){
  setcolorder(tab11,c("atc_code_1", "count","total", "meaning", "year", "data_access_provider", "data_source"))
    if(tab11[,.N]>12000){  
  print("If the table contanins more than 12000 records it can not be displayed in the report. For the complete table, go to g_output/VACCINES/vaccines_my_atc_1_f_masked.csv")
...
if(!is.null(tab11)){
tab11_graph<-tab11[count=="<5", count:= 3]
...
if (tab14[,.N]>0){
  setcolorder(tab14,c("atc_code_7", "no_records", "no_female_users", "median_rx_female_users","mean_rx_female_users", "meaning", "year", "atc_code_3", "data_access_provider", "data_source"))
  if (tab14[,.N]>12000){
        print("If the table contanins more than 12000 records it can not be displayed in the report. For the complete table, go to g_output/VACCINES/_vaccines_my_atc_7_f_masked.csv")
...
When subjects are being followed(i.e. they contribute to the person time in a particular year) but do not have aN administred vaccine, the truncated_atc_code variable will be set to N/A.
...
```{r tab15_graph_VACCINES, echo=FALSE, warning=FALSE}
if(!is.null(tab15)){
tab15_graph<-tab15[count_vaccines=="<5", count_vaccines:= 3]
...
tab15_graph<-tab15_graph[vaccines_per_100_py=="N/A", vaccines_per_100_py:= round((count_vaccines/person_years)*100,2)]
...
ggplotly(ggplot(tab15_graph, aes(x = year, y = vaccines_per_100_py, group=truncated_atc_code)) +
...
datatable(tab16, options = list(scrollX=T))  
    ...
    if(sum(grepl(list.files(paste0(vacc_dir,subpopulations_names[a], "/Masked/")), pattern="vaccines_rates_by_year_age_atc_masked.csv"))>0){
...
if(!is.null(tab16)){
tab16_graph<-tab16[count_vaccines=="<5", count_vaccines:= 3]
...
tab16_graph<-tab16_graph[vaccines_per_100_py=="N/A",vaccines_per_100_py:=round((count_vaccines/person_years)*100,2)]
...
fig.tab16_med[[i]]<-ggplotly(ggplot(tab16_graph[age_band==unique(tab16_graph[,age_band])[i]], aes(x = year, y = vaccines_per_100_py, group=truncated_atc_code)) +
                           geom_line(aes(color=truncated_atc_code)) +
                           geom_point(aes(color=truncated_atc_code)) + 
                               ggtitle(paste0("Rate of vaccine exposure in females of childbearing age, age band:", unique(tab16_graph[,age_band])[i])) + 
                               xlab("Year") +
                               ylab("Rate per 100 person-years")+
                             theme_classic() +
                             guides(shape = guide_legend(override.aes = list(size = 0.3))) +
...
calculations_vaccines<-data.table(rbind(
  ...
  cbind(indicator= "Number of records and users by ATC code, meaning and year",           variable_name=c("no_records", "no_male_users", "median_rx_male_users","no_female_users","median_rx_female_users"),
         stratifying_variable=c("vx_atc(truncated to first letter, third or fourth letter of the ATC code)",
                                "vx_atc(truncated to first letter, third or fourth letter of the ATC code)",
                                "vx_atc(truncated to first letter, third or fourth letter of the ATC code)",
                                "vx_atc(truncated to first letter, third or fourth letter of the ATC code)",
                                "vx_atc(truncated to first letter, third or fourth letter of the ATC code)",
                                "vx_atc(truncated to first letter, third or fourth letter of the ATC code), meaning of record and year",
                                "vx_atc(truncated to first letter, third or fourth letter of the ATC code), meaning of record and year","vx_atc(truncated to first letter, third or fourth letter of the ATC code), meaning of record and year","vx_atc(truncated to first letter, third or fourth letter of the ATC code), meaning of record and year","vx_atc(truncated to first letter, third or fourth letter of the ATC code), meaning of record and year"),
         calculation=c("Number of records for a particular medicinal group as defined by the first, third or fourth letter of the ATC code stratified(if a vaccine has an ATC code up to 3 letter then the atc_code_4 will show as N/A, same for ATC code up to one letter)", "Number of male users for a vaccine with a particular ATC code as defined by the number of letters of the ATC code", "Median number of records for male users for a vaccine with a particular ATC code as defined by the number of letters of the ATC code","Number of female users for a vaccine with a particular ATC code as defined by the number of letters of the ATC code", "Median number of records for female users for a vaccine with a particular ATC code as defined by the number of letters of the ATC code", "Number of records for a particular medicinal group as defined by the first, third or fourth letter of the ATC code stratified by the meaning of the record and year(if a vaccine has an ATC code up to 3 letter then the atc_code_4 will show as N/A, same for ATC code up to one letter)", "Number of male users for a vaccine with a particular ATC code as defined by the number of letters of the ATC code stratified by the meaning of the record and year", "Median number of records for male users for a vaccine with a particular ATC code as defined by the number of letters of the ATC code stratified by the meaning of the record and year","Number of female users for a vaccine with a particular ATC code as defined by the number of letters of the ATC code stratified by the meaning of the record and year", "Median number of records for female users for a vaccine with a particular ATC code as defined by the number of letters of the ATC code stratified by the meaning of the record and year")),
  cbind(indicator= "Number of records and users by ATC code, meaning and year(atc code up to seven letters)",           variable_name=c("no_records", "no_male_users", "median_rx_male_users","no_female_users","median_rx_female_users"),
         stratifying_variable=c("vx_atc",
                                "vx_atc",
                                "vx_atc",
                                "vx_atc",
                                "vx_atc",
                                "vx_atc, meaning of record and year",
                                "vx_atc, meaning of record and year","vx_atc, meaning of record and year","vx_atc, meaning of record and year","vx_atc, meaning of record and year"
                                ),
         calculation=c("Number of records for a particular medicinal group", "Number of male users for a vaccine", "Median number of records for male users for a vaccine","Number of female users for a vaccine", "Median number of records for female users for a vaccine", "Number of records for a particular medicinal group stratified by the meaning of the record and year", "Number of male users for a vaccine with a particular ATC code stratified by the meaning of the record and year", "Median number of records for male users for a vaccine stratified by the meaning of the record and year","Number of female users for a vaccine stratified by the meaning of the record and year", "Median number of records for female users for a vaccine stratified by the meaning of the record and year")),
  ...
  cbind(indicator= "Number of records and users by ATC code, meaning and year(atc code up to seven letters) in females of childbearing age(12-55 years old at start of follow up)",           variable_name=c("no_records", "no_female_users","median_rx_female_users"),
         stratifying_variable=c("vx_atc",
                                "vx_atc",
                                "vx_atc",
                                "vx_atc, meaning of record and year","vx_atc, meaning of record and year","vx_atc, meaning of record and year"
                                ),
         calculation=c("Number of records for a particular medicinal group in females of childbearing age(12-55 years old at start of follow up)", "Number of female users of childbearing age(12-55 years old at start of follow up) for a vaccine", "Median number of records for female users of childbearing age(12-55 years old at start of follow up) for a vaccine", "Number of records for a particular medicinal group stratified by the meaning of the record and year in females of childbearing age(12-55 years old at start of follow up)", "Number of female users of childbearing age(12-55 years old at start of follow up) for a vaccine stratified by the meaning of the record and year", "Median number of records for female users of childbearing age(12-55 years old at start of follow up) for a vaccine stratified by the meaning of the record and year")),
  cbind(indicator= "Rate of medicines prescriptions/dispensing by year in females of childbearing age(12-55 years old at start of follow up)",           variable_name=c("count_medicines", "no_users","no_subjects","person_years"),
         stratifying_variable=c("vx_atc(truncated to three letters) and year",
                                "vx_atc(truncated to three letters) and year",
                                "year",
                                "year"),
         calculation=c("Number of records for a particular medicinal group in females of childbearing age(12-55 years old at start of follow up) by year", "Number of female users of childbearing age(12-55 years old at start of follow up) who are dispensed/prescribed a vaccine in a particular year", "Number of female subjects of childbearing age(12-55 years old at start of follow up) who are present(i.e. contribute to the person time) in a particular year", "Follow up time by year in females of childbearing age(12-55 years old at start of follow up)" )),
  cbind(indicator= "Rate of medicines prescriptions/dispensing by year and age band in females of childbearing age(12-55 years old at start of follow up)",           variable_name=c("count_medicines", "no_users","no_subjects","person_years"),
         stratifying_variable=c("vx_atc(truncated to three letters), age_band and year",
                                "vx_atc(truncated to three letters), age_band and year",
                                "year and age_band",
                                "year and age_band"),
         calculation=c("Number of records for a particular medicinal group in females of childbearing age(12-55 years old at start of follow up) by year and age_band", "Number of female users of childbearing age(12-55 years old at start of follow up) who are dispensed/prescribed a vaccine in a particular year by age_band", "Number of female subjects of childbearing age(12-55 years old at start of follow up) who are present(i.e. contribute to the person time) in a particular year by age_band", "Follow up time by year and age_band in females of childbearing age(12-55 years old at start of follow up)" )),
