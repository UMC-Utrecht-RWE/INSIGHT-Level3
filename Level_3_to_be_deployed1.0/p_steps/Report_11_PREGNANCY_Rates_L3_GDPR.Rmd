...
subtitle: "GDPR: Rates of the Pregnancy study population"
output: 
  html_document:
    theme: spacelab
    toc: true
...
if (subpopulations_present=="No"){
  if(length(list.files(paste0(preg_dir,"GDPR/"), pattern="pregnancy_rates_ya_masked.csv"))>0){
preg_ya_rates<-fread(paste0(preg_dir,"GDPR/", list.files(paste0(preg_dir,"GDPR/"), pattern="pregnancy_rates_ya_masked.csv")))
if (preg_ya_rates[,.N]>0){
  if (preg_ya_rates[,.N]>12000){
    ...
    datatable(head(preg_ya_rates[no_records!=0],12000), options = list(scrollX=T), filter = "top")
  } else {
datatable(preg_ya_rates[no_records!=0], options = list(scrollX=T), filter = "top")
  }
} else {
  preg_ya_rates<-NULL
    ...
    if(length(list.files(paste0(preg_dir,subpopulations_names[a], "/GDPR/"), pattern="pregnancy_rates_ya_masked.csv"))>0){
  preg_ya_rates<-fread(paste0(preg_dir,subpopulations_names[a], "/GDPR/", list.files(paste0(preg_dir,subpopulations_names[a], "/GDPR/"), pattern="pregnancy_rates_ya_masked.csv")))
  if (preg_ya_rates[,.N]>0){
   if (preg_ya_rates[,.N]>12000){   
            print("If the table contanins more than 12000 records it can not be displayed in the report. For the complete table, go to g_output/PREGNANCY/pregnancy_rates_ya_masked.csv")
        datatable(head(preg_ya_rates[no_records!=0],12000), options = list(scrollX=T), filter = "top")
...
if (subpopulations_present=="No"){
  if(length(list.files(paste0(preg_dir,"GDPR/"), pattern="pregnancy_rates_y_masked.csv"))>0){
preg_y_rates<-fread(paste0(preg_dir,"GDPR/", list.files(paste0(preg_dir,"GDPR/"), pattern="pregnancy_rates_y_masked.csv")))
if (preg_y_rates[,.N]>0){
  if (preg_y_rates[,.N]>12000){
    ...
    datatable(head(preg_y_rates[no_records!=0],12000), options = list(scrollX=T), filter = "top")
  } else {
datatable(preg_y_rates[no_records!=0], options = list(scrollX=T), filter = "top")
  }
} else {
  preg_y_rates<-NULL
    print("This table cannot be produced based on the available data or this analysis was not performed.")
    ...
    if(length(list.files(paste0(preg_dir,subpopulations_names[a], "/GDPR/"), pattern="pregnancy_rates_y_masked.csv"))>0){
  preg_y_rates<-fread(paste0(preg_dir,subpopulations_names[a], "/GDPR/", list.files(paste0(preg_dir,subpopulations_names[a], "/GDPR/"), pattern="pregnancy_rates_y_masked.csv")))
  if (preg_y_rates[,.N]>0){
   if (preg_y_rates[,.N]>12000){   
            print("If the table contanins more than 12000 records it can not be displayed in the report. For the complete table, go to g_output/PREGNANCY/pregnancy_rates_y_masked.csv")
        datatable(head(preg_y_rates[no_records!=0],12000), options = list(scrollX=T), filter = "top")
