...
Line_plot2 <- function(MATRIX, title, x.l, y.l, x.axis = F, color = NULL, leg = T, x.labels = NULL, x.thicks = NULL, x.las = 2, point = F, l.size = 0.9, tck.size = 0.6){
  ...
  #X-axis
  if(x.axis==F){axis(1,at=1:length(colnames(MATRIX)),labels = colnames(MATRIX), las = x.las,cex.axis = tck.size)}
  if(x.axis==T){axis(1, at=x.thicks, labels = x.labels, las = x.las, cex.axis = tck.size)}
  
      
  if(ceiling(max(MATRIX)) < 10) t <- format(seq(from = 0,to = ceiling(max(MATRIX)),length.out = 11))
  if(ceiling(max(MATRIX)) >= 10)t <- format(seq(from = 0,to = ceiling(max(MATRIX)),length.out = 11))
  #axis(2, at = t , labels = as.character(t),cex.axis =0.6 )
