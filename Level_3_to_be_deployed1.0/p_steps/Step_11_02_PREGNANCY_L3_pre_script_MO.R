...
print("Analyse MEDICAL_OBSERVATIONS table for the PREGNANCY script.")
############################################
...
if(length(actual_tables$MEDICAL_OBSERVATIONS)>0){
  ...
  mo_date_miss_preg<-list() #number of record with missing pregnancy_code_date
  ...
  mo_code_vocabulary_miss_preg<-list() #number of records with both event code and event record vocabulary missing
  ...
  for (y in 1:length(actual_tables$MEDICAL_OBSERVATIONS)){
    ...
    pers_stdpop_not_mo_preg<-df[rowSums(is.na(df[,..colnames_mo]))==length(colnames_mo), ..std_names_mo] #subjects id present in the study population but that do not have an event
    ...
    mo_out_st_per_preg[[w]]<-df[pregnancy_code_date<start_follow_up_preg | pregnancy_code_date>end_follow_up_preg,.N] #number of records outside study population
    ...
    mo_code_vocabulary_miss_preg[[w]]<-df[is.na(pregnancy_code) & is.na(pregnancy_code_vocabulary),.N]#numbe rof records with both event code and vocabulary missing
    ...
    mo_code_pres_voc_miss_preg[[w]]<-df[!is.na(pregnancy_code) & is.na(pregnancy_code_vocabulary),.N] #number of records where event code present but vocabulary missing
    df[!is.na(pregnancy_code) & is.na(pregnancy_code_vocabulary), remove:=1]#new 02.06.2022
    df<-df[is.na(remove)]#new 02.06.2022
    df[,remove:=NULL]#new 02.06.2022
    mo_not_vocabularies_preg[[w]]<-df[pregnancy_code_vocabulary %!in% vocabularies_list_pregnancy,.N] #number of records where vocabularies doesn't match the codelist
    ...
    meanings_mo_preg[[w]]<-df[!duplicated(meaning), "meaning"]
    #years_mo_preg[[w]]<-unique(na.omit(df[, year])) #will be used for description
    years_mo_preg[[w]]<-df[!duplicated(year), "year"] #will be used for description
    ...
    if(df[,.N]>0){
      ...
      pers_included_mo<-df[!duplicated(person_id), c("person_id","birth_date","start_follow_up_preg","end_follow_up_preg","sex_at_instance_creation")]
      
      if(pers_included_mo[,.N]>0){
        saveRDS(pers_included_mo, paste0(preg_m_tmp, paste0("pers_included_mo_preg_", actual_tables$MEDICAL_OBSERVATIONS[y], ".rds"))) #to give the results we will merge the files by keeping only person_id that are in all saved files
      }
      rm(pers_included_mo)
      
      ####match codes based on coding system and code: algorithm start with####
      print(paste0("Extracting data for pregnancy_to_start_with:",actual_tables$MEDICAL_OBSERVATIONS[y]))
      if(sum(df[!duplicated(pregnancy_code_vocabulary), pregnancy_code_vocabulary] %in% pregnancy_to_start_with)>0){
        ...
        for (i in 1:length(stage_pregnancy_start)){
          ...
          for(j in 1:length(stage_pregnancy_start[[i]])){
            ...
            #       df[,code_no_dot:=NULL]
            #     }
            #     saveRDS(data.table(df[filter==T & year==years_study_events_preg[m]], condition=names(stage_pregnancy_start[i])), paste0(preg_m_tmp,years_study_events_preg[m],"_", names(stage_pregnancy_start[i]), "_",actual_tables$MEDICAL_OBSERVATIONS[y], "_start.rds"))
            #   }
            #   m<-m+1
            ...
            if(df[!is.na(filter),.N]>0){
              ...
              for(year_ind in 1:length(years_this_event)){
                ...
                saveRDS(data.table(df[,c("person_id","pregnancy_code_date","meaning","sex_at_instance_creation","birth_date","start_follow_up_preg","end_follow_up_preg","year","condition","filter")][!is.na(filter) & year==years_this_event[year_ind]], condition=names(stage_pregnancy_start[i])), paste0(preg_m_tmp,years_this_event[year_ind],"_", names(stage_pregnancy_start[i]), "_",actual_tables$MEDICAL_OBSERVATIONS[y], "_start.rds"))
              }
            } else{
      ...
      print(paste0("Extracting data for pregnancy_rcd:",actual_tables$MEDICAL_OBSERVATIONS[y]))
      if(sum(df[!duplicated(pregnancy_code_vocabulary), pregnancy_code_vocabulary] %in% pregnancy_rcd)>0){
        ...
        for (i in 1:length(stage_pregnancy_read)){
          ...
          for(j in 1:length(stage_pregnancy_read[[i]])){
            ...
            #   repeat{
            #     if(df[filter==T & year==years_study_events_preg[m],.N]>0){
            #       saveRDS(data.table(df[filter==T & year==years_study_events_preg[m]], condition=names(stage_pregnancy_read[i])), paste0(preg_m_tmp,years_study_events_preg[m],"_", names(stage_pregnancy_read[i]), "_",actual_tables$MEDICAL_OBSERVATIONS[y], "_RCD.rds"))
            #     }
            #     m<-m+1
            #     if(m >length(years_study_events_preg)){
            #       break
            ...
            if(df[filter==T,.N]>0){
              ...
              for(year_ind in 1:length(years_this_event)){
                #col_rm<-c("code_no_dot")
                saveRDS(data.table(df[,c("person_id","pregnancy_code_date","meaning","sex_at_instance_creation","birth_date","start_follow_up_preg","end_follow_up_preg","year","condition","filter")][filter==T & year==years_this_event[year_ind]], condition=names(stage_pregnancy_read[i])), paste0(preg_m_tmp,years_this_event[year_ind],"_", names(stage_pregnancy_read[i]), "_",actual_tables$MEDICAL_OBSERVATIONS[y], "_RCD.rds"))
              }
            } else {
      ...
      print(paste0("Extracting data for pregnancy_snomed:",actual_tables$MEDICAL_OBSERVATIONS[y]))
      if(sum(df[!duplicated(pregnancy_code_vocabulary), pregnancy_code_vocabulary] %in% pregnancy_snomed_codes)>0){
        ...
        for (i in 1:length(stage_pregnancy_snomed)){
          ...
          for(j in 1:length(stage_pregnancy_snomed[[i]])){
            ...
            #     saveRDS(data.table(df[filter_preg==1 & year==years_study_events_preg[m]], condition=names(stage_pregnancy_snomed[i])), paste0(preg_m_tmp,years_study_events_preg[m],"_", names(stage_pregnancy_snomed[i]), "_",actual_tables$MEDICAL_OBSERVATIONS[y], "_SNOMED.rds"))
            #   }
            #   m<-m+1
            #   if(m >length(years_study_events_preg)){
            #     break
            ...
            if(df[filter==T,.N]>0){
              years_this_event<-sort(df[filter==T][!duplicated(year),year])
              for(year_ind in 1:length(years_this_event)){
                saveRDS(data.table(df[,c("person_id","pregnancy_code_date","meaning","sex_at_instance_creation","birth_date","start_follow_up_preg","end_follow_up_preg","year","condition","filter")][filter==T & year==years_this_event[year_ind]], condition=names(stage_pregnancy_snomed[i])), paste0(preg_m_tmp,years_this_event[year_ind],"_", names(stage_pregnancy_snomed[i]), "_",actual_tables$MEDICAL_OBSERVATIONS[y], "_SNOMED.rds"))
              }
            } else {
              years_this_event<-NULL}# new 01.06.2022
      ...
      print(paste0("Extracting data for pregnancy_other:",actual_tables$MEDICAL_OBSERVATIONS[y]))
      if(sum(df[!duplicated(pregnancy_code_vocabulary), pregnancy_code_vocabulary] %in% pregnancy_other_codes)>0){
        ...
        for (i in 1:length(stage_pregnancy_other)){
          ...
          for(j in 1:length(stage_pregnancy_other[[i]])){
            ...
            #     saveRDS(data.table(df[filter==T & year==years_study_events[m]], condition=names(stage_pregnancy_other[i])), paste0(events_tmp,years_study_events[m],"_", names(stage_pregnancy_other[i]), "_",actual_tables$MEDICAL_OBSERVATIONS[y], "_other.rds"))
            #   }
            #   m<-m+1
            ...
            if(df[filter==T,.N]>0){
              years_this_event<-sort(df[filter==T][!duplicated(year),year])
              for(year_ind in 1:length(years_this_event)){
                saveRDS(data.table(df[,c("person_id","pregnancy_code_date","meaning","sex_at_instance_creation","birth_date","start_follow_up_preg","end_follow_up_preg","year","condition","filter")][filter==T & year==years_this_event[year_ind]], condition=names(stage_pregnancy_other[i])), paste0(preg_m_tmp,years_this_event[year_ind],"_", names(stage_pregnancy_other[i]), "_",actual_tables$MEDICAL_OBSERVATIONS[y], "_other.rds"))
              }
            } else {
              years_this_event<-NULL}# new 01.06.2022
  ...
  stdpop_not_mo_files_preg<-list.files(preg_m_tmp, pattern = "stdpop_not_mo_preg_")
  if (length(stdpop_not_mo_files_preg)>0){
    mo_not_id<-readRDS(paste0(preg_m_tmp, stdpop_not_mo_files_preg[1]))
    i<-2
    while(i <= length(stdpop_not_mo_files_preg)){
      a<-readRDS(paste0(preg_m_tmp, stdpop_not_mo_files_preg[i]))
    ...
    for(i in 1:length(stdpop_not_mo_files_preg)){
      unlink(paste0(preg_m_tmp,stdpop_not_mo_files_preg[i]))
    }
    
    study_population[person_id %in% mo_not_id[,person_id], no_mo_id:=1]
    
    if (mo_not_id[,.N]>0){
      saveRDS(mo_not_id, paste0(preg_m_tmp, "mo_not_ids_stdpop_preg.rds"))
    }
    stdpop_not_mo_preg<-mo_not_id[,.N]
    rm(mo_not_id)
  } else {
    stdpop_not_mo_preg<-0
  }
  rm(stdpop_not_mo_files_preg)
  
  
  ####subjects included in the events_study population####
  stdpop_mo_files_preg<-list.files(preg_m_tmp, pattern = "pers_included_mo_preg_")
  if (length(stdpop_mo_files_preg)>0){
    mo_included_id<-readRDS(paste0(preg_m_tmp, stdpop_mo_files_preg[1]))
    i<-2
    while(i <= length(stdpop_mo_files_preg)){
      a<-readRDS(paste0(preg_m_tmp, stdpop_mo_files_preg[i]))
    ...
    for(i in 1:length(stdpop_mo_files_preg)){
      unlink(paste0(preg_m_tmp,stdpop_mo_files_preg[i]))
    }
    
    
    if (mo_included_id[,.N]>0){
      saveRDS(mo_included_id, paste0(preg_m_tmp, "pers_mo_preg.rds"))
    }
    rm(mo_included_id)
  }
  rm(stdpop_mo_files_preg)
  ...
  rm(orig_no_rows_mo_preg, mo_excluded_meanings_preg,
     mo_study_pop_preg,mo_date_miss_preg,mo_out_st_per_preg,
  ...
  meanings_mo_preg<-meanings_mo_preg[!duplicated(meaning),meaning]
  ...
  years_mo_preg<-sort(years_mo_preg[!duplicated(year),year])
