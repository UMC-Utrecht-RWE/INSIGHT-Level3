...
subtitle: "Overview and completeness of the Pregnancy study population"
output: 
  html_document:
    theme: spacelab
    toc: true
...
```{r echo=F}
if (subpopulations_present=="No"){
flowchart_pregnancy<-fread(paste0(preg_dir,"Masked/",list.files(paste0(preg_dir,"Masked/"), "pregnancy_flowchart_masked.csv")))
datatable(flowchart_pregnancy, options = list(scrollX=T))
} else {
flowchart_pregnancy<-fread(paste0(preg_dir,subpopulations_names[a], "/Masked/",list.files(paste0(preg_dir,subpopulations_names[a], "/Masked/"), "pregnancy_flowchart_masked.csv")))
datatable(flowchart_pregnancy, options = list(scrollX=T))
}
```
...
```{r echo=F}
if (subpopulations_present=="No"){
description_pregnancy<-fread(paste0(preg_dir,"Masked/",list.files(paste0(preg_dir,"Masked/"), "pregnancy_description_masked.csv")))
datatable(description_pregnancy, options = list(scrollX=T))
} else {
  description_pregnancy<-fread(paste0(preg_dir,subpopulations_names[a], "/Masked/", list.files(paste0(preg_dir,subpopulations_names[a], "/Masked/"),"pregnancy_description_masked.csv")))
datatable(description_pregnancy, options = list(scrollX=T))
}
```
...
```{r echo=F}
if (subpopulations_present=="No"){
  if(length(list.files(paste0(preg_dir,"Masked/"), pattern="pregnancy_records_per_women_masked.csv"))>0){
preg_rec_women<-fread(paste0(preg_dir,"Masked/", list.files(paste0(preg_dir,"Masked/"), pattern="pregnancy_records_per_women_masked.csv")))
if(preg_rec_women[,.N]>0){
datatable(preg_rec_women, options = list(scrollX=T))
  } else {
    preg_rec_women<-NULL
  ...
  print("This table cannot be produced based on the available data.")
    ...
    if(length(list.files(paste0(preg_dir,subpopulations_names[a], "/Masked/"), pattern="pregnancy_records_per_women_masked.csv"))>0){
    preg_rec_women<-fread(paste0(preg_dir,subpopulations_names[a], "/Masked/", list.files(paste0(preg_dir,subpopulations_names[a], "/Masked/"), pattern="pregnancy_records_per_women_masked.csv")))
    if(preg_rec_women[,.N]>0){
datatable(preg_rec_women, options = list(scrollX=T))
    } else {
      preg_rec_women<-NULL
...
preg_rec_women_graph<-preg_rec_women[`1_pregnancy_record`=="<5", `1_pregnancy_record`:= 3]
preg_rec_women_graph<-preg_rec_women_graph[,`1_pregnancy_record`:=as.numeric(`1_pregnancy_record`)]
preg_rec_women_graph<-preg_rec_women_graph[`2_pregnancy_records`=="<5", `2_pregnancy_records`:= 3]
preg_rec_women_graph<-preg_rec_women_graph[,`2_pregnancy_records`:=as.numeric(`2_pregnancy_records`)]
preg_rec_women_graph<-preg_rec_women_graph[`3_pregnancy_records`=="<5", `3_pregnancy_records`:= 3]
preg_rec_women_graph<-preg_rec_women_graph[,`3_pregnancy_records`:=as.numeric(`3_pregnancy_records`)]
preg_rec_women_graph<-preg_rec_women_graph[`more_than_4_pregnancy_records`=="<5", `more_than_4_pregnancy_records`:= 3]
preg_rec_women_graph<-preg_rec_women_graph[,`more_than_4_pregnancy_records`:=as.numeric(`more_than_4_pregnancy_records`)]

graph_preg<-suppressWarnings(data.table(melt(preg_rec_women_graph,id.vars = c("year","event_definition"),
                measure.vars = c("1_pregnancy_record", "2_pregnancy_records", "3_pregnancy_records","more_than_4_pregnancy_records"))))
setnames(graph_preg,"value","count")
setnames(graph_preg,"variable","presence of codes category")
}

if(!is.null(preg_rec_women)){
  if(graph_preg[,.N]>0){
ggplotly(ggplot(graph_preg, aes(fill=`presence of codes category`, y=year, x=count)) + 
...
```{r echo=F}
if (subpopulations_present=="No"){
  if(length(list.files(paste0(preg_dir,"Masked/"), pattern="pregnancy_combined_records_masked.csv"))>0){
preg_rec_comb<-fread(paste0(preg_dir,"Masked/", list.files(paste0(preg_dir,"Masked/"), pattern="pregnancy_combined_records_masked.csv")))
...
datatable(preg_rec_comb, options = list(scrollX=T))
  ...
  print("This table cannot be produced based on the available data.")
    ...
    if(length(list.files(paste0(preg_dir,subpopulations_names[a], "/Masked/"), pattern="pregnancy_combined_records_masked.csv"))>0){
    preg_rec_comb<-fread(paste0(preg_dir,subpopulations_names[a], "/Masked/", list.files(paste0(preg_dir,subpopulations_names[a], "/Masked/"), pattern="pregnancy_combined_records_masked.csv")))
    if(preg_rec_comb[,.N]>0){
datatable(preg_rec_comb, options = list(scrollX=T))
    } else {
      preg_rec_comb<-NULL
...
preg_rec_comb_graph<-preg_rec_comb[no_women=="<5", no_women:= 3]
preg_rec_comb_graph<-preg_rec_comb_graph[,no_women:=as.numeric(no_women)]
preg_rec_comb_graph<-preg_rec_comb_graph[total_women=="<5", total_women:= 3]
preg_rec_comb_graph<-preg_rec_comb_graph[,total_women:=as.numeric(total_women)]

#recalculate the percentage
preg_rec_comb_graph[,percentage_no_women:=round((no_women/total_women)*100,2)]
} else {
 preg_rec_comb_graph<-NULL 
}


if(!is.null(preg_rec_comb_graph)){
# Compute the cumulative percentages (top of each rectangle)
  if(preg_rec_comb_graph[,.N]>0){
ggplot(preg_rec_comb_graph, aes(y=percentage_no_women, x=event_definition)) + 
      geom_bar(stat="identity", color="darkgreen", fill="#76b82a")+
  geom_text(aes(label=percentage_no_women),size=3, vjust=1.6) +
    ggtitle("Combination of pregnancy records") +
  ylab("Percentage") +
  theme(axis.text.y = element_blank(),
    ...
    print("The graph cannot be displayed.")
  }
} else {
    print("This table cannot be produced based on the available data.")
  }
