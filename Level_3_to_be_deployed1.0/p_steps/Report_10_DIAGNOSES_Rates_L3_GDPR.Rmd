...
subtitle: "GDPR: Counts of the Diagnoses study population"
output: 
  html_document:
    theme: spacelab
    toc: true
...
if (subpopulations_present=="No"){
  if(length(list.files(paste0(diag_dir,"GDPR/"), pattern="diagnoses_rates_yas_recurrent_masked.csv"))>0){
rates_yas_rec<-fread(paste0(diag_dir,"GDPR/", list.files(paste0(diag_dir,"GDPR/"), pattern="diagnoses_rates_yas_recurrent_masked.csv")))
...
if (rates_yas_rec[,.N]>0){
  if (rates_yas_rec[,.N]>12000){
    ...
    datatable(head(rates_yas_rec[no_records!=0],12000), options = list(scrollX=T), filter = "top")
  } else {
datatable(rates_yas_rec[no_records!=0], options = list(scrollX=T), filter = "top")
  }
} else {
  rates_yas_rec<-NULL
    ...
    if(length(list.files(paste0(diag_dir,subpopulations_names[a], "/GDPR/"), pattern="diagnoses_rates_yas_recurrent_masked.csv"))>0){
  rates_yas_rec<-fread(paste0(diag_dir,subpopulations_names[a], "/GDPR/", list.files(paste0(diag_dir,subpopulations_names[a], "/GDPR/"), pattern="diagnoses_rates_yas_recurrent_masked.csv")))
  setcolorder(rates_yas_rec, c("event_definition","sex","age_band","year","no_records","person_years","rate_per_100_py"))
  if (rates_yas_rec[,.N]>0){
   if (rates_yas_rec[,.N]>12000){   
            print("If the table contanins more than 12000 records it can not be displayed in the report. For the complete table, go to g_output/DIAGNOSES/diagnoses_rates_yas_recurrent_masked.csv")
...
if (subpopulations_present=="No"){
  if(length(list.files(paste0(diag_dir,"GDPR/"), pattern="diagnoses_rates_ys_recurrent_masked.csv"))>0){
rates_ys_rec<-fread(paste0(diag_dir,"GDPR/", list.files(paste0(diag_dir,"GDPR/"), pattern="diagnoses_rates_ys_recurrent_masked.csv")))
...
if (rates_ys_rec[,.N]>0){
  if (rates_ys_rec[,.N]>12000){
    ...
    datatable(head(rates_ys_rec[no_records!=0],12000), options = list(scrollX=T), filter = "top")
  } else {
datatable(rates_ys_rec[no_records!=0], options = list(scrollX=T), filter = "top")
  }
} else {
  rates_ys_rec<-NULL
    print("This table cannot be produced based on the available data or this analysis was not performed.")
    ...
    if(length(list.files(paste0(diag_dir,subpopulations_names[a], "/GDPR/"), pattern="diagnoses_rates_ys_recurrent_masked.csv"))>0){
  rates_ys_rec<-fread(paste0(diag_dir,subpopulations_names[a], "/GDPR/", list.files(paste0(diag_dir,subpopulations_names[a], "/GDPR/"), pattern="diagnoses_rates_ys_recurrent_masked.csv")))
  setcolorder(rates_ys_rec, c("event_definition","sex","year","no_records","person_years","rate_per_100_py"))
  if (rates_ys_rec[,.N]>0){
   if (rates_ys_rec[,.N]>12000){   
            print("If the table contanins more than 12000 records it can not be displayed in the report. For the complete table, go to g_output/DIAGNOSES/diagnoses_rates_ys_recurrent_masked.csv")
...
if (subpopulations_present=="No"){
  if(length(list.files(paste0(diag_dir,"GDPR/"), pattern="diagnoses_rates_y_recurrent_masked.csv"))>0){
rates_y_rec<-fread(paste0(diag_dir,"GDPR/", list.files(paste0(diag_dir,"GDPR/"), pattern="diagnoses_rates_y_recurrent_masked.csv")))
...
if (rates_y_rec[,.N]>0){
  if (rates_y_rec[,.N]>12000){
    ...
    datatable(head(rates_y_rec[no_records!=0],12000), options = list(scrollX=T), filter = "top")
  } else {
datatable(rates_y_rec[no_records!=0], options = list(scrollX=T), filter = "top")
  }
} else {
  rates_y_rec<-NULL
    print("This table cannot be produced based on the available data or this analysis was not performed.")
    ...
    if(length(list.files(paste0(diag_dir,subpopulations_names[a], "/GDPR/"), pattern="diagnoses_rates_y_recurrent_masked.csv"))>0){
  rates_y_rec<-fread(paste0(diag_dir,subpopulations_names[a], "/GDPR/", list.files(paste0(diag_dir,subpopulations_names[a], "/GDPR/"), pattern="diagnoses_rates_y_recurrent_masked.csv")))
  setcolorder(rates_y_rec, c("event_definition","year","no_records","person_years","rate_per_100_py"))
  if (rates_y_rec[,.N]>0){
   if (rates_y_rec[,.N]>12000){   
            print("If the table contanins more than 12000 records it can not be displayed in the report. For the complete table, go to g_output/DIAGNOSES/diagnoses_rates_y_recurrent_masked.csv")
...
if (subpopulations_present=="No"){
  if(length(list.files(paste0(diag_dir,"GDPR/"), pattern="diagnoses_rates_yas_first_masked.csv"))>0){
rates_yas_first<-fread(paste0(diag_dir,"GDPR/", list.files(paste0(diag_dir,"GDPR/"), pattern="diagnoses_rates_yas_first_masked.csv")))
...
if (rates_yas_first[,.N]>0){
  if (rates_yas_first[,.N]>12000){
    ...
    datatable(head(rates_yas_first[no_records!=0],12000), options = list(scrollX=T), filter = "top")
  } else {
datatable(rates_yas_first[no_records!=0], options = list(scrollX=T), filter = "top")
  }
} else {
  rates_yas_first<-NULL
    ...
    if(length(list.files(paste0(diag_dir,subpopulations_names[a], "/GDPR/"), pattern="diagnoses_rates_yas_first_masked.csv"))>0){
  rates_yas_first<-fread(paste0(diag_dir,subpopulations_names[a], "/GDPR/", list.files(paste0(diag_dir,subpopulations_names[a], "/GDPR/"), pattern="diagnoses_rates_yas_first_masked.csv")))
  setcolorder(rates_yas_first, c("event_definition","sex","age_band","year","no_records","person_years","rate_per_100_py"))
  if (rates_yas_first[,.N]>0){
   if (rates_yas_first[,.N]>12000){   
            print("If the table contanins more than 12000 records it can not be displayed in the report. For the complete table, go to g_output/DIAGNOSES/diagnoses_rates_yas_first_masked.csv")
...
if (subpopulations_present=="No"){
  if(length(list.files(paste0(diag_dir,"GDPR/"), pattern="diagnoses_rates_ys_first_masked.csv"))>0){
rates_ys_first<-fread(paste0(diag_dir,"GDPR/", list.files(paste0(diag_dir,"GDPR/"), pattern="diagnoses_rates_ys_first_masked.csv")))
...
if (rates_ys_first[,.N]>0){
  if (rates_ys_first[,.N]>12000){
    ...
    datatable(head(rates_ys_first[no_records!=0],12000), options = list(scrollX=T), filter = "top")
  } else {
datatable(rates_ys_first[no_records!=0], options = list(scrollX=T), filter = "top")
  }
} else {
  rates_ys_first<-NULL
    print("This table cannot be produced based on the available data or this analysis was not performed.")
    ...
    if(length(list.files(paste0(diag_dir,subpopulations_names[a], "/GDPR/"), pattern="diagnoses_rates_ys_first_masked.csv"))>0){
  rates_ys_first<-fread(paste0(diag_dir,subpopulations_names[a], "/GDPR/", list.files(paste0(diag_dir,subpopulations_names[a], "/GDPR/"), pattern="diagnoses_rates_ys_first_masked.csv")))
  setcolorder(rates_ys_first, c("event_definition","sex","year","no_records","person_years","rate_per_100_py"))
  if (rates_ys_first[,.N]>0){
   if (rates_ys_first[,.N]>12000){   
            print("If the table contanins more than 12000 records it can not be displayed in the report. For the complete table, go to g_output/DIAGNOSES/diagnoses_rates_ys_first_masked.csv")
...
if (subpopulations_present=="No"){
  if(length(list.files(paste0(diag_dir,"GDPR/"), pattern="diagnoses_rates_y_first_masked.csv"))>0){
rates_y_first<-fread(paste0(diag_dir,"GDPR/", list.files(paste0(diag_dir,"GDPR/"), pattern="diagnoses_rates_y_first_masked.csv")))
...
if (rates_y_first[,.N]>0){
  if (rates_y_first[,.N]>12000){
    ...
    datatable(head(rates_y_first[no_records!=0],12000), options = list(scrollX=T), filter = "top")
  } else {
datatable(rates_y_first[no_records!=0], options = list(scrollX=T), filter = "top")
  }
} else {
  rates_y_first<-NULL
    print("This table cannot be produced based on the available data or this analysis was not performed.")
    ...
    if(length(list.files(paste0(diag_dir,subpopulations_names[a], "/GDPR/"), pattern="diagnoses_rates_y_first_masked.csv"))>0){
  rates_y_first<-fread(paste0(diag_dir,subpopulations_names[a], "/GDPR/", list.files(paste0(diag_dir,subpopulations_names[a], "/GDPR/"), pattern="diagnoses_rates_y_first_masked.csv")))
  setcolorder(rates_y_first, c("event_definition","year","no_records","person_years","rate_per_100_py"))
  if (rates_y_first[,.N]>0){
   if (rates_y_first[,.N]>12000){   
            print("If the table contanins more than 12000 records it can not be displayed in the report. For the complete table, go to g_output/DIAGNOSES/diagnoses_rates_y_first_masked.csv")
...
datatable(codes, options = list(scrollX=T), filter = "top")
```
...
datatable(calculations_diagnoses, filter = "top")
```
