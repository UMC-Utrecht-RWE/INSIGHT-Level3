...
subtitle: "GDPR: Pregnancy records after a diagnosis"
output: 
  html_document:
    theme: spacelab
    toc: true
...
```{r echo=F, warning=FALSE}
if (subpopulations_present=="No"){
  if(length(list.files(paste0(poi_dir,"GDPR/"), pattern="diag_preg_prior_removed.csv"))>0){
...
datatable(res, options = list(scrollX=T), filter="top")
  ...
  print("This table cannot be produced based on the available data.")
    ...
    if(sum(grepl(list.files(paste0(poi_dir,"EVENTS_PREGNANCY/",subpopulations_names[a], "/GDPR/")), pattern="diag_preg_prior_removed.csv"))>0){
...
```{r echo=F, warning=FALSE}
if (subpopulations_present=="No"){
  if(length(list.files(paste0(poi_dir,"GDPR/"), pattern="diag_preg_prevalence_y_masked.csv"))>0){
fl<-paste0(poi_dir,"GDPR/")
res_1<-fread(paste0(fl,list.files(fl,"diag_preg_prevalence_y_masked.csv")))
if(res_1[,.N]>0){
datatable(res_1, options = list(scrollX=T), filter="top")
  } else {
    res_1<-NULL
  print("This table cannot be produced based on the available data.")
  }
  ...
  print("This table cannot be produced based on the available data.")
    ...
    if(sum(grepl(list.files(paste0(poi_dir,"EVENTS_PREGNANCY/",subpopulations_names[a], "/GDPR/")), pattern="diag_preg_prevalence_y_masked.csv"))>0){
      fl<-paste0(poi_dir,"EVENTS_PREGNANCY/",subpopulations_names[a], "/GDPR/")
...
if(!is.null(res_1)){
rec_graph<-as.data.table(res_1[,.(event_definition, year, stage_of_pregnancy,prevalence_pregnancies)])
...
```{r echo=F, warning=FALSE}
if (subpopulations_present=="No"){
  if(sum(grepl(list.files(paste0(poi_dir,"GDPR/")), pattern="diag_preg_counts_y_masked.csv"))>0){
fl<-paste0(poi_dir,"GDPR/")
res_2<-fread(paste0(fl,list.files(fl,"diag_preg_counts_y_masked.csv")))
if(res_2[,.N]>0){
datatable(res_2, options = list(scrollX=T), filter="top")
  } else {
    res_2<-NULL
  print("This table cannot be produced based on the available data.")
  }
  ...
  print("This table cannot be produced based on the available data.")
    ...
    if(sum(grepl(list.files(paste0(poi_dir,"EVENTS_PREGNANCY/",subpopulations_names[a], "/GDPR/")), pattern="diag_preg_counts_y_masked.csv"))>0){
      fl<-paste0(poi_dir,"EVENTS_PREGNANCY/",subpopulations_names[a], "/GDPR/")
...
if(!is.null(res_2)){
rec_graph<-res_2[,.(event_definition,stage_of_pregnancy,year,percentage)]
rec_graph[,year:=as.factor(year)]
...
```{r echo=F, warning=FALSE}
if (subpopulations_present=="No"){
  if(length(list.files(paste0(poi_dir,"GDPR/"), pattern="diag_preg_counts_ym_masked.csv"))>0){
fl<-paste0(poi_dir,"GDPR/")
res_3<-fread(paste0(fl,list.files(fl,"diag_preg_counts_ym_masked.csv")))
if(res_3[,.N]>0){
datatable(res_3, options = list(scrollX=T),filter="top")
  } else {
    res_3<-NULL
  print("This table cannot be produced based on the available data.")
  }
  ...
  print("This table cannot be produced based on the available data.")
    ...
    if(sum(grepl(list.files(paste0(poi_dir,"EVENTS_PREGNANCY/",subpopulations_names[a], "/GDPR/")), pattern="diag_preg_counts_ym_masked.csv"))>0){
      fl<-paste0(poi_dir,"EVENTS_PREGNANCY/",subpopulations_names[a], "/GDPR/")
...
```{r , echo=FALSE, warning=FALSE}
if(!is.null(res_3)){
rec_graph<-res_3[,.(event_definition,stage_of_pregnancy,meaning,year,percentage)]
rec_graph<-rec_graph[,year:=as.factor(year)]
