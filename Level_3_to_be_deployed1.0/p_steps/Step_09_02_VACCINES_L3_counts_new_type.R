...
####Export table 15####  
  ...
  if(!is.null(tab15)){
    ...
    ####Export masked and GDPR version table 15####
  #Apply masking
...
####Table 10: Number of administrations/record by ATC A level in the study population by year of administration/record and by meaning####
  ...
  #create percentage
  tab10[,percentage:=round((count/total)*100,1)]
  setcolorder(tab10,c("meaning","year","vaccine_indicator","count","total","percentage"))
####Export table 10####  
  ...
  if(!is.null(tab10)){
    ...
    tab10<-as.data.table(tab10)
####Export GDPR version table 10####
    tab10[,count:=as.numeric(count)][,total:=as.numeric(total)]
    tab10[records_categories, c_range := i.range[count >= min & count <= max], on = .(count >= min, count <= max)]
    tab10[records_categories, t_range := i.range[total >= min & total <= max], on = .(total >= min, total <= max)]
    
    gdpr_file<-tab10[,-c("count","total"),with=F]
    setnames(gdpr_file,"c_range","count")
    setnames(gdpr_file,"t_range","total")
    setcolorder(gdpr_file,c("meaning","year","vaccine_indicator","count","total","percentage"))

    tab10[,c_range:=NULL][,t_range:=NULL]
  if(subpopulations_present=="Yes"){
    fwrite(gdpr_file, paste0(vacc_dir,subpopulations_names[s], "/","GDPR/",date_DAP_name_part, subpopulations_names[s],"_vaccines_my_atc_1_masked.csv"), row.names = F)
  } else {
    fwrite(gdpr_file, paste0(vacc_dir,"GDPR/",date_DAP_name_part,"vaccines_my_atc_1_masked.csv"), row.names = F)
  }
  
  ####Export masked version table 10####
  #Apply masking
...
####Table 11: Number of administrations/record by ATC A level in the female study population of childbearing age 12-55 years (based on age at Start_study_fup) by year of administration/record and by meaning####
  ...
  tab11[,percentage:=round((count/total)*100,1)]
  setcolorder(tab11,c("meaning","year","vaccine_indicator","count","total","percentage"))
  print("Export table 11.")
####Export table 11#### 
  if(!is.null(tab11)){
  ...
  ####Export GDPR version table 11####
  #Create gdpr file
    tab11[,count:=as.numeric(count)][,total:=as.numeric(total)]
    tab11[records_categories, c_range := i.range[count >= min & count <= max], on = .(count >= min, count <= max)]
    tab11[records_categories, t_range := i.range[total >= min & total <= max], on = .(total >= min, total <= max)]
    
    gdpr_file<-tab11[,-c("count","total"),with=F]
    setnames(gdpr_file,"c_range","count")
    setnames(gdpr_file,"t_range","total")
    setcolorder(gdpr_file,c("meaning","year","vaccine_indicator","count","total","percentage"))
    
    tab11[,c_range:=NULL][,t_range:=NULL]
...
####Export table 12####
  ...
  if(!is.null(tab12)){
    ...
    setcolorder(gdpr_file,c("sex","year","atc_4", "no_records","no_users","median_records","mean_records", "meaning"))
        ...
        if(subpopulations_present=="Yes"){
      fwrite(gdpr_file, paste0(vacc_dir,subpopulations_names[s], "/","GDPR/",date_DAP_name_part, subpopulations_names[s],"_vaccines_my_atc_4_masked.csv"), row.names = F)
    } else {
      fwrite(gdpr_file, paste0(vacc_dir,"GDPR/",date_DAP_name_part, "vaccines_my_atc_4_masked.csv"), row.names = F)
    }

    tab12[,rec_range:=NULL][,us_range:=NULL]
    #Apply masking
  ####Export masked version table 12####
