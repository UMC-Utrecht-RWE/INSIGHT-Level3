...
for(i in names(Lifestyle)){
  ...
  if(!is.null(Lifestyle[[i]][["c.voc"]]) & !is.na(Lifestyle[[i]][["c.voc"]])) temp <- copy(get(Lifestyle[[i]][["CDM_table"]]))[get(Lifestyle[[i]][["CDM_column"]]) %in% eval(Lifestyle[[i]][["value"]]) & get(Lifestyle[[i]][["c.voc"]]) %in% eval(Lifestyle[[i]][["v.voc"]]),]
  if(is.null(Lifestyle[[i]][["c.voc"]]) | is.na(Lifestyle[[i]][["c.voc"]]))  temp <- copy(get(Lifestyle[[i]][["CDM_table"]]))[get(Lifestyle[[i]][["CDM_column"]]) %in% eval(Lifestyle[[i]][["value"]]) ,]
...
Agebands <- CreateBands(seq(from = Age_min , to = Age_max, by = 10))
...
for(i in 1:nrow(SCHEME_0113)){
  ...
  for(j in names(Lifestyle)){ 
    ...
    if(!is.null(Lifestyle[[i]][["c.voc"]]) & !is.na(Lifestyle[[i]][["c.voc"]])) temp <- copy(get(table))[get(Lifestyle[[j]][["CDM_column"]]) %in% eval(Lifestyle[[j]][["value"]]) & get(Lifestyle[[j]][["c.voc"]]) %in% eval(Lifestyle[[j]][["v.voc"]]),]
    if(is.null(Lifestyle[[i]][["c.voc"]]) | is.na(Lifestyle[[i]][["c.voc"]]))  temp <- copy(get(table))[get(Lifestyle[[j]][["CDM_column"]]) %in% eval(Lifestyle[[j]][["value"]]) ,]
  ...
  new <- c("Factor","Year op_start_date","Ageband start follow up","Subject with at least 1 record in study period","Subject in category","%","Order" )
