...
if (subpopulations_present=="No"){
  if(length(list.files(paste0(diag_dir,"GDPR/"), pattern="diagnoses_counts_m_masked.csv"))>0){
tab21_diag<-fread(paste0(diag_dir,"GDPR/",list.files(paste0(diag_dir,"GDPR/"), "diagnoses_counts_m_masked.csv")))
...
datatable(head(tab21_diag,12000), options = list(scrollX=T), filter = "top")
 } else {
        datatable(tab21_diag, options = list(scrollX=T), filter = "top")
 }
} else {
  tab21_diag<-NULL
  ...
  print("This table cannot be produced based on the available data.")
    ...
    if(length(list.files(paste0(diag_dir,subpopulations_names[a], "/GDPR/"), pattern="diagnoses_counts_m_masked.csv"))>0){
  tab21_diag<-fread(paste0(diag_dir,subpopulations_names[a], "/GDPR/",list.files(paste0(diag_dir,subpopulations_names[a], "/GDPR/"), "_diagnoses_counts_m_masked.csv")))
